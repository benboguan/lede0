...
static NDIS_STATUS l1set_profile_path(RTMP_ADAPTER *pAd, UINT_32 extra,
				      RTMP_STRING *value)
{
	...
	if (pSemicolon) {
		...
		*pSemicolon = '\0';
		...
		update_mtb_value(
			pAd, MTB_2G_PROFILE, extra,
			value); /* force update mtb[] to prevent unsynced value */
#endif /* MULTI_PROFILE */
	}
...
static NDIS_STATUS l1set_eeprom_bin(RTMP_ADAPTER *pAd, UINT_32 extra,
				    RTMP_STRING *value)
{
	INT retVal = NDIS_STATUS_SUCCESS;
	RTMP_STRING *target =
		l1profile[get_dev_config_idx(pAd)].ee_info.bin_name;
...
static NDIS_STATUS l1set_eeprom_offset(RTMP_ADAPTER *pAd, UINT_32 extra,
				       RTMP_STRING *value)
{
	INT retVal = NDIS_STATUS_SUCCESS;
	eeprom_flash_info *target = (eeprom_flash_info *)(&(
		l1profile[get_dev_config_idx(pAd)].ee_info));
	UINT int_value = 0;
...
static NDIS_STATUS l1set_eeprom_size(RTMP_ADAPTER *pAd, UINT_32 extra,
				     RTMP_STRING *value)
{
	UINT int_value;
	INT retVal = NDIS_STATUS_SUCCESS;
...
static NDIS_STATUS l1set_ifname(RTMP_ADAPTER *pAd, UINT_32 extra,
				RTMP_STRING *value)
{
	INT retVal = NDIS_STATUS_SUCCESS;
...
#ifdef SINGLE_SKU_V2
static NDIS_STATUS l1set_single_sku_path(RTMP_ADAPTER *pAd, UINT_32 extra,
					 RTMP_STRING *value)
{
	INT retVal = NDIS_STATUS_SUCCESS;
	RTMP_STRING *target =
		l1profile[get_dev_config_idx(pAd)].single_sku_path;
...
static NDIS_STATUS l1set_bf_sku_path(RTMP_ADAPTER *pAd, UINT_32 extra,
				     RTMP_STRING *value)
{
	INT retVal = NDIS_STATUS_SUCCESS;
...
static NDIS_STATUS is_dup_key(RTMP_STRING *key)
{
	...
	for (dev_idx = 0; dev_idx < MAX_NUM_OF_INF; dev_idx++) {
		...
		if ((strlen(profile_index) > 0) &&
		    (strcmp(profile_index, key) == 0)) {
	...
	while (if_idx < MAX_L1PROFILE_INDEX) {
		...
		if (RTMPGetKeyParameter(key, tmpbuf, MAX_PARAM_BUFFER_SIZE,
			...
			if (strncmp(tmpbuf, chipName, strlen(chipName)) == 0) {
				...
				if (is_dup_key(
					    key)) { /* There might be not only single entry for one chip */
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL,
						 DBG_LVL_TRACE,
						 ("%s for %s occupied, next\n",
						  key, chipName));
				} else {
					strncpy(l1profile[get_dev_config_idx(
								  pAd)]
							.profile_index,
						key, L1PROFILE_INDEX_LEN - 1);
					retVal = NDIS_STATUS_SUCCESS;
					if_idx =
						MAX_L1PROFILE_INDEX; /* found, intend to leave */
				}
			} else {
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL,
					 DBG_LVL_TRACE,
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				 ("%s not found, dismissed.\n", key));
			if_idx =
				MAX_L1PROFILE_INDEX; /* hit maximum avalable index, intend to leave */
		}
	}
...
static NDIS_STATUS l1get_profile_index(IN RTMP_STRING *l1profile_data,
				       IN RTMP_ADAPTER *pAd)
{
	INT retVal = NDIS_STATUS_SUCCESS;
	...
	if (match_index_by_chipname(l1profile_data, pAd, chipName) ==
	    NDIS_STATUS_SUCCESS) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("[%d]%s found by chip\n", dev_idx, chipName));
	} else {
...
int ShowL1profile(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("===== L1 profile settings =====\n"));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("%s = %s\n", l1profile_attributes[0].name,
		  get_dev_l2profile(pAd)));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("%s = %x\n", l1profile_attributes[2].name,
		  get_dev_eeprom_offset(pAd)));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("%s = %x\n", l1profile_attributes[3].name,
		  get_dev_eeprom_size(pAd)));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		 ("%s = %s\n", l1profile_attributes[4].name,
		  get_dev_name_prefix(pAd, INT_MAIN)));
...
NDIS_STATUS load_dev_l1profile(IN RTMP_ADAPTER *pAd)
{
	...
	if (retval) {
		...
		if ((strlen(profile_index) == 0) &&
		    (l1get_profile_index(buffer, pAd) != NDIS_STATUS_SUCCESS))
			goto err_out1;
		...
		if (RTMPGetKeyParameter(profile_index, tmpbuf,
					MAX_PARAM_BUFFER_SIZE, buffer, TRUE)) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				 ("Chip=%s\n", tmpbuf));
			...
			for (attr_index = 0;
				...
				snprintf(key, sizeof(key), "%s_%s",
					 profile_index, l1attr->name);

				if (RTMPGetKeyParameter(key, tmpbuf,
...
NDIS_STATUS RTMPReadParametersHook(RTMP_ADAPTER *pAd)
{
...
#endif /* OS_ABL_SUPPORT */
				retval = os_file_read(srcf, buffer,
						      buf_size - 1);
				if (retval > 0) {
					RTMPSetProfileParameters(pAd, buffer);
					retval = NDIS_STATUS_SUCCESS;
	...
	for (i = 0; i < pAd->ApCfg.BssidNum; i++) {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("Reset ra%d hostapd support=FLASE", i));
	}
	...
	if (src && *src) {
		...
		if (srcf.Status)
			...
			if (buffer) {
				...
				if (retval > 0) {
					RTMPSetPreProfileParameters(pAd,
								    buffer);
					retval = NDIS_STATUS_SUCCESS;
				} else
					MTWF_LOG(
			     ...
			     IN NDIS_MEDIA_STATE media_state)
{
	...
	if (pAd->IndicateMediaState == NdisMediaStateConnected) {
		...
		RTMPSendWirelessEvent(pAd, IW_STA_LINKUP_EVENT_FLAG,
				      pAd->MacTab.Content[wcid].Addr, BSS0, 0);
	} else {
		UINT wcid = BSSID_WCID_TO_REMOVE; /* Pat: TODO */
...
VOID ApCliLinkCoverRxPolicy(IN PRTMP_ADAPTER pAd, IN PNDIS_PACKET pPacket,
			    OUT BOOLEAN *DropPacket)
{
	...
	if (wf_drv_tbl.wf_fwd_feedback_map_table)
		wf_drv_tbl.wf_fwd_feedback_map_table(
			pAd, &band_tbl, &opp_band_tbl, &other_band_tbl);
	...
	if (IS_GROUP_MAC(pPktHdr)) {
		...
		if (opp_band_tbl != NULL)
			pOtherBandReptEntry = RTMPLookupRepeaterCliEntry(
				opp_band_tbl, FALSE, pPktHdr + 6, FALSE);

		if (other_band_tbl != NULL)
			pAnotherBandReptEntry = RTMPLookupRepeaterCliEntry(
				other_band_tbl, FALSE, pPktHdr + 6, FALSE);

		if ((pInvalidEntry != NULL) || (pOtherBandReptEntry != NULL) ||
		    (pAnotherBandReptEntry != NULL)) {
			MTWF_LOG(
			   ...
			   IN UCHAR OpMode)
{
	...
	PNDIS_PACKET pRxPkt = pPacket;
#if defined(WH_EZ_SETUP) &&                                                    \
	(defined(CONFIG_WIFI_PKT_FWD) || defined(CONFIG_WIFI_PKT_FWD_MODULE))
	...
	if (pAd->ApCfg.ApCliTab[0].wdev.wds_enable == 0) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			if (RTMP_MATPktRxNeedConvert(
				    pAd, RtmpOsPktNetDevGet(pRxPkt))) {
#if defined(CONFIG_WIFI_PKT_FWD) || defined(CONFIG_WIFI_PKT_FWD_MODULE)

				if ((wf_drv_tbl.wf_fwd_needed_hook != NULL) &&
				    (wf_drv_tbl.wf_fwd_needed_hook() == TRUE)) {
					BOOLEAN need_drop = FALSE;

					ApCliLinkCoverRxPolicy(pAd, pPacket,
							       &need_drop);
...
#ifdef INF_PPA_SUPPORT
	{
		if (ppa_hook_directpath_send_fn && (pAd->PPAEnable == TRUE)) {
			...
			if (retVal > 0) {
				ret = ppa_hook_directpath_send_fn(pAd->g_if_id,
								  pRxPkt,
								  pRxPkt->len,
								  ppa_flags);
				...
				if ((mh->h_dest[0] & 0x1) == 0x1) {
					if (RTMP_IS_PACKET_APCLI(pOsRxPkt)) {
...
#endif /* MAC_REPEATER_SUPPORT */
						{
							...
							if (band_tbl != NULL) {
								if (MAC_ADDR_EQUAL(
									    ((UCHAR *)((REPEATER_ADAPTER_DATA_TABLE
												*)
											       band_tbl)
										     ->Wdev_ifAddr),
									    mh->h_source) ||
...
INT Monitor_VirtualIF_Close(PNET_DEV dev_p)
{
	...
	RTMP_COM_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_SNIFF_CLOSE, 0, dev_p,
			     0);
...
VOID RT28xx_Monitor_Init(VOID *pAd, PNET_DEV main_dev_p)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		 ("%s: %d !!!!####!!!!!!\n", __func__, __LINE__));
	RTMP_COM_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_SNIFF_INIT, 0,
			     &netDevOpHook, 0);
}
VOID RT28xx_Monitor_Remove(VOID *pAd)
{
	RTMP_COM_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_SNIFF_REMOVE, 0, NULL,
			     0);
}
...
void STA_MonPktSend(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, UCHAR DevIdx)
{
	...
	if (sniffer_type == RADIOTAP_TYPE) {
		if (pRxBlk->DataSize + sizeof(struct mtk_radiotap_header) >
		    pAd->monitor_ctrl[DevIdx].FilterSize) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
	...
	MaxRssi = RTMPMaxRssi(
		...
		ConvertToRssi(
			pAd,
			(struct raw_rssi_info *)(&pRxBlk->rx_signal.raw_rssi[0]),
			RSSI_IDX_0),
		ConvertToRssi(
			pAd,
			(struct raw_rssi_info *)(&pRxBlk->rx_signal.raw_rssi[0]),
	...
	if (sniffer_type == RADIOTAP_TYPE) {
		RssiForRadiotap = RTMPMaxRssi(
			pAd,
			ConvertToRssi(
				pAd,
				(struct raw_rssi_info *)(&pRxBlk->rx_signal
								  .raw_rssi[0]),
				RSSI_IDX_0),
			ConvertToRssi(
				pAd,
...
#else
	pNetDev =
		pAd->monitor_ctrl[DevIdx].wdev.if_dev; /* send packet to mon0 */
#endif
	...
	BssMonitorFlag11n =
		(pAd->StaCfg[0].BssMonitorFlag & MONITOR_FLAG_11N_SNIFFER);
	...
	if (BW == BW_80)
		sideband_index =
			vht_prim_ch_idx(CentralChannel, Channel, RF_BW_80);

#ifdef SNIFFER_MT7615
	else if (BW == BW_160)
		sideband_index =
			vht_prim_ch_idx(CentralChannel, Channel, RF_BW_160);
	...
	if (sniffer_type == RADIOTAP_TYPE) {
		...
		if (IS_MT7615(pAd)) {
			gid = ((pRxBlk->rmac_info[46] >> 5) & 0x7) |
			      ((pRxBlk->rmac_info[47] & 0x7) << 3);
			...
			send_radiotap_mt7615_monitor_packets(
				pNetDev, pRxBlk->rmac_info, pRxBlk->rxv2_cyc1,
				pRxPacket, pData, DataSize, pDevName,
				RssiForRadiotap, UP_value);
		} else
#endif
			send_radiotap_monitor_packets(
				pNetDev, pRxBlk->AmsduState, pRxBlk->rmac_info,
				pRxPacket, (void *)dot11_fc_field, pData,
				DataSize, L2PAD, PHYMODE, BW, ShortGI, MCS,
				LDPC, LDPC_EX_SYM, AMPDU, STBC, RSSI1, pDevName,
				Channel, CentralChannel, sideband_index,
				RssiForRadiotap, timestamp, UP_value);
	}

	if (sniffer_type == PRISM_TYPE) {
		send_prism_monitor_packets(pNetDev, pRxPacket,
					   (void *)dot11_fc_field, pData,
					   DataSize, L2PAD, PHYMODE, BW,
					   ShortGI, MCS, AMPDU, STBC, RSSI1,
					   BssMonitorFlag11n, pDevName, Channel,
					   CentralChannel, MaxRssi);
	}
...
VOID RTMPFreeAdapter(VOID *pAdSrc)
{
	...
	hdev_ctrl_exit(pAd);
	RtmpOsVfree(
		pAd); /* pci_free_consistent(os_cookie->pci_dev,sizeof(RTMP_ADAPTER),pAd,os_cookie->pAd_pa); */
...
int RTMPSendPackets(IN NDIS_HANDLE dev_hnd, IN PPNDIS_PACKET pkt_list,
		    IN UINT pkt_cnt, IN UINT32 pkt_total_len,
		    IN RTMP_NET_ETH_CONVERT_DEV_SEARCH Func)
{
	...
	if (pkt_total_len < 14) {
		hex_dump("bad packet", GET_OS_PKT_DATAPTR(pPacket),
			 pkt_total_len);
...
INT RTMP_AP_IoctlPrepare(RTMP_ADAPTER *pAd, VOID *pCB)
{
	...
	if ((pConfig->priv_flags == INT_MAIN) &&
	    !RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_REGISTER_TO_OS)) {
	...
	if (pConfig->priv_flags == INT_MAIN) {
		...
		pObj->pSecConfig =
			&pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.SecConfig;
		...
		if (strcmp(pConfig->name, RtmpOsGetNetDevName(pAd->net_dev)) !=
		    0) { /* sample */
		...
		for (index = 0; index < MAX_WDS_ENTRY; index++) {
			if (pAd->WdsTab.WdsEntry[index].wdev.if_dev ==
			    pConfig->net_dev) {
				pObj->ioctl_if = index;
				pObj->pSecConfig = &pAd->WdsTab.WdsEntry[index]
							    .wdev.SecConfig;
				break;
			}
		...
		for (index = 0; index < MAX_APCLI_NUM; index++) {
			if (pAd->ApCfg.ApCliTab[index].wdev.if_dev ==
			    pConfig->net_dev) {
				pObj->ioctl_if = index;
				pObj->pSecConfig = &pAd->ApCfg.ApCliTab[index]
							    .wdev.SecConfig;
				break;
			}
...
VOID AP_E2PROM_IOCTL_PostCtrl(IN RTMP_IOCTL_INPUT_STRUCT *wrq,
			      IN RTMP_STRING *msg)
{
...
#ifdef WDS_SUPPORT
VOID AP_WDS_KeyNameMakeUp(IN RTMP_STRING *pKey, IN UINT32 KeyMaxSize,
			  IN INT KeyId)
{
...
VOID wifi_fwd_register(struct wifi_fwd_func_table *table)
{
	...
	wf_drv_tbl.wf_fwd_set_debug_level_hook =
		table->wf_fwd_set_debug_level_hook;
	...
	wf_drv_tbl.wf_fwd_access_schedule_halt_hook =
		table->wf_fwd_access_schedule_halt_hook;
	...
	wf_drv_tbl.packet_source_show_entry_hook =
		table->packet_source_show_entry_hook;
	wf_drv_tbl.packet_source_delete_entry_hook =
		table->packet_source_delete_entry_hook;
	...
	wf_drv_tbl.wf_fwd_insert_bridge_mapping_hook =
		table->wf_fwd_insert_bridge_mapping_hook;
	wf_drv_tbl.wf_fwd_insert_repeater_mapping_hook =
		table->wf_fwd_insert_repeater_mapping_hook;
	wf_drv_tbl.wf_fwd_search_mapping_table_hook =
		table->wf_fwd_search_mapping_table_hook;
	wf_drv_tbl.wf_fwd_delete_entry_inform_hook =
		table->wf_fwd_delete_entry_inform_hook;
	wf_drv_tbl.wf_fwd_check_device_hook = table->wf_fwd_check_device_hook;
	wf_drv_tbl.wf_fwd_add_entry_inform_hook =
		table->wf_fwd_add_entry_inform_hook;
	wf_drv_tbl.wf_fwd_set_easy_setup_mode =
		table->wf_fwd_set_easy_setup_mode;
