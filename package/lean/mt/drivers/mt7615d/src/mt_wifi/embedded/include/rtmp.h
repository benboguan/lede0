...
enum { IGMP_TVM_MODE_DISABLE = 0, IGMP_TVM_MODE_ENABLE, IGMP_TVM_MODE_AUTO };
...
typedef enum _MNT_BAND_TYPE { MNT_BAND0 = 1, MNT_BAND1 = 2 } MNT_BAND_TYPE;
...
#define OPSTATUS_TEST_FLAG(_pAd, _F)                                           \
	(((_pAd)->CommonCfg.OpStatusFlags & (_F)) != 0)
...
#define OPSTATUS_TEST_FLAG_WDEV(_Wdev, _F)                                     \
	(((_Wdev)->OpStatusFlags & (_F)) != 0)
#define OPSTATUS_EQUAL_FLAG_WDEV(_Wdev, _F)                                    \
	(((_Wdev)->OpStatusFlags & (_F)) == (_F))

#define WIFI_TEST_SET_FLAG(_pAd, _F) ((_pAd)->CommonCfg.WiFiTestFlags |= (_F))
#define WIFI_TEST_CLEAR_FLAG(_pAd, _F)                                         \
	((_pAd)->CommonCfg.WiFiTestFlags &= ~(_F))
#define WIFI_TEST_CHECK_FLAG(_pAd, _F)                                         \
	(((_pAd)->CommonCfg.WiFiTestFlags & (_F)) != 0)

#define CLIENT_STATUS_SET_FLAG(_pEntry, _F)                                    \
	((_pEntry)->ClientStatusFlags |= (_F))
#define CLIENT_STATUS_CLEAR_FLAG(_pEntry, _F)                                  \
	((_pEntry)->ClientStatusFlags &= ~(_F))
#define CLIENT_STATUS_TEST_FLAG(_pEntry, _F)                                   \
	(((_pEntry)->ClientStatusFlags & (_F)) != 0)
...
#define CLIENT_CAP_TEST_FLAG(_pEntry, _F)                                      \
	(((_pEntry)->cli_cap_flags & (_F)) != 0)

#define RX_FILTER_SET_FLAG(_pAd, _F) ((_pAd)->CommonCfg.PacketFilter |= (_F))
#define RX_FILTER_CLEAR_FLAG(_pAd, _F) ((_pAd)->CommonCfg.PacketFilter &= ~(_F))
#define RX_FILTER_TEST_FLAG(_pAd, _F)                                          \
	(((_pAd)->CommonCfg.PacketFilter & (_F)) != 0)
...
#define IS_ASIC_CAP(_pAd, _caps)                                               \
	((hc_get_asic_cap(_pAd->hdev_ctrl) & (_caps)) != 0)
...
#define TX_FLAG_STOP_DEQUEUE                                                   \
	(fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS | fRTMP_ADAPTER_HALT_IN_PROGRESS | \
	 fRTMP_ADAPTER_NIC_NOT_EXIST | fRTMP_ADAPTER_DISABLE_DEQUEUEPACKET)
...
typedef struct _MT_MIB_BUCKET_ONE_SEC {
	...
	UINT32 ChannelBusyTime
		[DBDC_BAND_NUM]; /* Every second update once. primary channel only*/
	UINT32 ChannelBusyTimeCcaNavTx
		[DBDC_BAND_NUM]; /* Every second update once. including CCA NAV TX time*/
...
/*
	Some utility macros
*/
#define GET_LNA_GAIN(_pAd)                                                     \
	((_pAd->LatchRfRegs.Channel <= 14) ?                                   \
		 (_pAd->BLNAGain) :                                            \
		       ((_pAd->LatchRfRegs.Channel <= 64) ?                          \
			  (_pAd->ALNAGain0) :                                  \
				((_pAd->LatchRfRegs.Channel <= 128) ?                \
				   (_pAd->ALNAGain1) :                         \
					 (_pAd->ALNAGain2))))
...
#define LEAP_CCKM_ON(_p)                                                       \
	((((_p)->StaCfg[0].LeapAuthMode) == CISCO_AuthModeLEAP) &&             \
	 ((_p)->StaCfg[0].LeapAuthInfo.CCKM == TRUE))
...
#define MAKE_802_3_HEADER(_buf, _pMac1, _pMac2, _pType)                        \
	{                                                                      \
		...
		NdisMoveMemory((_buf + MAC_ADDR_LEN * 2), _pType,              \
			       LENGTH_802_3_TYPE);                             \
	}
	...
	{                                                                      \
		...
		if (NdisEqualMemory(SNAP_802_1H, _pData, 6) ||                 \
			...
			PUCHAR pProto = _pData + 6;                            \
                                                                               \
			if ((NdisEqualMemory(IPX, pProto, 2) ||                \
			     NdisEqualMemory(APPLE_TALK, pProto, 2)) &&        \
			    NdisEqualMemory(SNAP_802_1H, _pData, 6)) {         \
				LLC_Len[0] = (UCHAR)(_DataSize >> 8);          \
				LLC_Len[1] = (UCHAR)(_DataSize & (256 - 1));   \
				MAKE_802_3_HEADER(_p8023hdr, _pDA, _pSA,       \
						  LLC_Len);                    \
			} else {                                               \
				MAKE_802_3_HEADER(_p8023hdr, _pDA, _pSA,       \
						  pProto);                     \
...
#define REPORT_MGMT_FRAME_TO_MLME(_pAd, Wcid, _pFrame, _FrameSize, _Rssi0,     \
				  _Rssi1, _Rssi2, _Rssi3, _MinSNR, _channel,   \
				  _OpMode, _wdev, _RxPhyMode)                  \
	do {                                                                   \
		...
		MlmeEnqueueForRecv(_pAd, Wcid, &_rssi_info, _FrameSize,        \
				   _pFrame, _OpMode, _wdev, _RxPhyMode);       \
	} while (0)
#else
#define REPORT_MGMT_FRAME_TO_MLME(_pAd, Wcid, _pFrame, _FrameSize, _Rssi0,     \
				  _Rssi1, _Rssi2, _Rssi3, _Snr0, _Snr1, _Snr2, \
				  _Snr3, _MinSNR, _channel, _OpMode, _wdev,    \
				  _RxPhyMode)                                  \
	do {                                                                   \
		struct raw_rssi_info _rssi_info;                               \
		_rssi_info.raw_rssi[0] = _Rssi0;                               \
...
enum { OUI_MGROUP_ACT_JOIN = 0, OUI_MGROUP_ACT_LEAVE = 1 };
#define MAC_OUI_EQUAL(pAddr1, pAddr2)                                          \
	RTMPEqualMemory((PVOID)(pAddr1), (PVOID)(pAddr2), 4)
#endif /*OUI_CHECK_SUPPORT*/
#define IPV4_ADDR_EQUAL(pAddr1, pAddr2)                                        \
	RTMPEqualMemory((PVOID)(pAddr1), (PVOID)(pAddr2), 4)
#define IPV6_ADDR_EQUAL(pAddr1, pAddr2)                                        \
	RTMPEqualMemory((PVOID)(pAddr1), (PVOID)(pAddr2), 16)
#define MAC_ADDR_EQUAL(pAddr1, pAddr2)                                         \
	RTMPEqualMemory((PVOID)(pAddr1), (PVOID)(pAddr2), MAC_ADDR_LEN)
#define SSID_EQUAL(ssid1, len1, ssid2, len2)                                   \
	((len1 == len2) && (RTMPEqualMemory(ssid1, ssid2, len1)))
...
typedef struct _CHANNEL_CTRL {
	CHANNEL_TX_POWER ChList
		[MAX_NUM_OF_CHANNELS]; /* list all supported channels for site survey */
...
#endif
	enum BCN_TX_STATE
		bcn_state; /* Make sure if no packet pending in the Hardware */
...
#define IS_HT_RATE(_pMacEntry) (_pMacEntry->HTPhyMode.field.MODE >= MODE_HTMIX)

#ifdef DOT11_VHT_AC
#define IS_VHT_RATE(_pMacEntry) (_pMacEntry->HTPhyMode.field.MODE == MODE_VHT)
#endif
...
#define WDEV_WITH_BCN_ABILITY(_wdev)                                           \
	(((_wdev)->wdev_type == WDEV_TYPE_AP) ||                               \
...
#define MSDU_FORBID_SET(_wdev, _reason)                                        \
	(OS_SET_BIT(_reason, &((_wdev)->forbid_data_tx)))
#define MSDU_FORBID_CLEAR(_wdev, _reason)                                      \
	(OS_CLEAR_BIT(_reason, &((_wdev)->forbid_data_tx)))
...
struct wifi_dev_ops {
	...
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	 PNDIS_PACKET pkt) ____cacheline_aligned;
	INT(*fp_send_data_pkt)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev, PNDIS_PACKET pkt);
	INT(*send_mlme_pkt)
	(struct _RTMP_ADAPTER *pAd, PNDIS_PACKET pkt, struct wifi_dev *wdev,
	 UCHAR q_idx, BOOLEAN is_data_queue);
	INT(*tx_pkt_allowed)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev, PNDIS_PACKET pkt);
	INT(*fp_tx_pkt_allowed)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev, PNDIS_PACKET pkt);
	INT(*tx_pkt_handle)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	...
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	 struct _TX_BLK *tx_blk);
	BOOLEAN(*fill_offload_tx_blk)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	 struct _TX_BLK *tx_blk);
	BOOLEAN(*fill_non_offload_tx_blk)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	 struct _TX_BLK *tx_blk);
	VOID(*ieee_802_11_data_tx)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	 struct _TX_BLK *tx_blk);
	VOID(*ieee_802_3_data_tx)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	 struct _TX_BLK *tx_blk);
	...
	INT(*rx_pkt_allowed)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	 struct _RX_BLK *rx_blk) ____cacheline_aligned;
	INT(*rx_pkt_hdr_chk)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	...
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	 struct _RX_BLK *rx_blk);
	INT(*ieee_802_11_data_rx)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	 struct _RX_BLK *rx_blk, struct _MAC_TABLE_ENTRY *entry);
	INT(*ieee_802_3_data_rx)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	 struct _RX_BLK *rx_blk, struct _MAC_TABLE_ENTRY *entry);
	INT(*ieee_802_11_mgmt_rx)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	 struct _RX_BLK *rx_blk);
	INT(*ieee_802_11_ctl_rx)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	 struct _RX_BLK *rx_blk);
	INT(*rx_pkt_foward)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev, PNDIS_PACKET pkt);
	VOID(*find_cipher_algorithm)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
	 struct _TX_BLK *tx_blk);
	VOID(*detect_wmm_traffic)
	(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UCHAR up,
	 UCHAR flg_is_output);
	...
	INT(*disconn_act)
	(struct wifi_dev *wdev, struct _MAC_TABLE_ENTRY *entry);
	VOID(*mac_entry_lookup)
	(RTMP_ADAPTER *pAd, UCHAR *pAddr, struct wifi_dev *wdev,
...
enum {
	...
	QUICK_CH_SWICH_ENABLE_W_DISCONNECTION,
	QUICK_CH_SWICH_ENABLE_WO_DISCONNECTION,
};
...
struct wifi_dev {
	...
	DESIRED_TRANSMIT_SETTING
	DesiredTransmitSetting; /* Desired transmit setting. this is for reading registry setting only. not useful. */
	BOOLEAN bAutoTxRateSwitch;
	HTTRANSMIT_SETTING HTPhyMode, MaxHTPhyMode,
		MinHTPhyMode; /* For transmit phy setting in TXWI. */
...
#define WLAN_MR_TIM_BCMC_CLEAR(apidx)                                          \
	(pAd->ApCfg.MBSSID[apidx]                                              \
		 .wdev.bcn_buf.TimBitmaps[WLAN_CT_TIM_BCMC_OFFSET] &=          \
	 ~NUM_BIT8[0])
...
#define WLAN_MR_TIM_BIT_CLEAR(ad_p, apidx, _aid)                               \
	{                                                                      \
		...
		ad_p->ApCfg.MBSSID[apidx]                                      \
			.wdev.bcn_buf.TimBitmaps[tim_offset] &=                \
			(~NUM_BIT8[bit_offset]);                               \
	}
...
typedef struct _BSS_STRUCT {
	...
	*/
	...
	UCHAR DesiredRates
		[MAX_LEN_OF_SUPPORTED_RATES]; /* OID_802_11_DESIRED_RATES */
...
`		WPS segment
	*/
	...
	UCHAR ipv6LinkLocalSrcAddr
		[16]; /* Ipv6 link local address for this MBSS as per it's BSSID*/
...
typedef struct _COMMON_CONFIG {
	...
	REG_TRANSMIT_SETTING
	RegTransmitSetting; /*registry transmit setting. this is for reading registry setting only. not useful. */
	...
	UCHAR SKUTableIdx;
	CHAR cTxPowerCompBackup[DBDC_BAND_NUM][SKU_TABLE_SIZE]
			       [SKU_TX_SPATIAL_STREAM_NUM];
...
#ifdef DOT11_N_SUPPORT
	BACAP_STRUC
	BACapability; /*   NO USE = 0XFF  ;  IMMED_BA =1  ;  DELAY_BA=0 */
	BACAP_STRUC
	REGBACapability; /*   NO USE = 0XFF  ;  IMMED_BA =1  ;  DELAY_BA=0 */
#endif /* DOT11_N_SUPPORT */
...
#endif /* DOT11_N_SUPPORT */
	QOS_CAPABILITY_PARM
	APQosCapability; /* QOS capability of the current associated AP */
	EDCA_PARM APEdcaParm
		[WMM_NUM]; /* EDCA parameters of the current associated AP */
	QBSS_LOAD_PARM APQbssLoad; /* QBSS load of the current associated AP */
	UCHAR AckPolicy
		[WMM_NUM_OF_AC]; /* ACK policy of the specified AC. see ACK_xxx */
	...
	NEW_EXT_CHAN_IE
	NewExtChanOffset; /*7.3.2.20A, 1 if extension channel is above the control channel, 3 if below, 0 if not present */

	EXT_CAP_INFO_ELEMENT
	ExtCapIE; /* this is the extened capibility IE appreed in MGMT frames. Doesn't need to update once set in Init. */
...
#ifdef MICROWAVE_OVEN_SUPPORT
	MO_CFG_STRUCT
	MO_Cfg; /* data structure for mitigating microwave interference */
#endif /* MICROWAVE_OVEN_SUPPORT */
...
typedef struct _STA_TR_ENTRY {
	...
	*/
	...
	UINT32 HeaderBuf
		[32]; /* (total 128B) TempBuffer for TX_INFO + TX_WI + 802.11 Header + padding + AMSDU SubHeader + LLC/SNAP */
	...
	USHORT BARecWcidArray
		[NUM_OF_TID]; /* The mapping wcid of recipient session. if RXBAbitmap bit is masked */
	USHORT BAOriWcidArray
		[NUM_OF_TID]; /* The mapping wcid of originator session. if TXBAbitmap bit is masked */
	USHORT BAOriSequence
		[NUM_OF_TID]; /* The mapping wcid of originator session. if TXBAbitmap bit is masked */
...
struct fils_info {
	...
	VOID(*pending_decrypt)
	(PRTMP_ADAPTER pAd, struct _MAC_TABLE_ENTRY *pEntry,
...
typedef struct _MAC_TABLE_ENTRY {
	...
	*/
	...
	HTTRANSMIT_SETTING HTPhyMode,
		MaxHTPhyMode; /* For transmit phy setting in TXWI. */
	...
	*/
	...
	SST Sst;
	AUTH_STATE
	AuthState; /* for SHARED KEY authentication state machine used only */
	...
	PrivacyFilter; /* PrivacyFilter enum for 802.1X */
	...
	BOOLEAN toTxMrq;
	UCHAR msiToTx,
		mrqCnt; /*mrqCnt is used to count down the inverted-BF mrq to be sent */
	/* Rx mfb */
	UCHAR pendingMfsi;
	/* Tx MFB */
	BOOLEAN toTxMfb;
...
#ifdef CONFIG_MAP_SUPPORT
	UCHAR assoc_req_frame[ASSOC_REQ_LEN_MAX];
	...
	struct timeval tx_latency_min_tv;
	struct timeval tx_latency_max_tv;
	struct timeval tx_latency_avg_tv;
...
typedef struct _MAC_TABLE {
	...
	*/
	...
	BOOLEAN fAllStationAsRalink
		[2]; /* Check if all stations are ralink-chipset */
...
typedef struct _REPEATER_CTRL_STRUCT {
	INVAILD_TRIGGER_MAC_ENTRY
	IgnoreAsRepeaterEntry[MAX_IGNORE_AS_REPEATER_ENTRY_NUM];
...
typedef struct _APCLI_STRUCT {
	...
	*/
	...
	UCHAR ReqVarIEs
		[MAX_VIE_LEN]; /* The content saved here should be little-endian format. */
...
typedef struct _AP_ADMIN_CONFIG {
...
#ifdef APCLI_AUTO_CONNECT_SUPPORT
	APCLI_CONNECT_SCAN_TYPE ApCliAutoConnectType
		[MAX_APCLI_NUM]; /* 0 : User Trigger SCAN Mode, 1 :  Driver Trigger SCAN Mode, this is for Sigma DUT test , Peer AP may change BSSID, but SSID is the same */
	...
	ChannelSel_Alg
		AutoChannelAlg[DBDC_BAND_NUM]; /* Alg for selecting Channel */
#ifdef ACS_CTCC_SUPPORT
	BOOLEAN auto_ch_score_flag; /* score for Channel, and don't switch channel */
#endif
#ifdef AP_SCAN_SUPPORT
	UINT32 ACSCheckTime
		[DBDC_BAND_NUM]; /* Periodic timer to trigger Auto Channel Selection (unit: second) */
	UINT32 ACSCheckCount
		[DBDC_BAND_NUM]; /* if  ACSCheckCount > ACSCheckTime, then do ACS check */
	...
	UCHAR TVModeType
		[DBDC_BAND_NUM]; /* 0:Disable, 1:Enable, or 2:Auto, Valid only for AP wdev*/
#endif /* IGMP_TVM_SUPPORT */
...
typedef struct _CONFIGURATION_OF_TX_POWER_CONTROL_OVER_MAC {
	...
	*PCONFIGURATION_OF_TX_POWER_CONTROL_OVER_MAC;
...
typedef enum { WOW_GPIO_LOW_TO_HIGH, WOW_GPIO_HIGH_TO_LOW } WOW_GPIO_HIGH_LOW_T;
...
#define WOW_GPIO_LOW_TO_HIGH_PARAMETER                                         \
	((WOW_GPIO_OUTPUT_ENABLE << 0) | (WOW_GPIO_OUTPUT_LEVEL_LOW << 1) |    \
	 (WOW_GPIO_WAKEUP_LEVEL_HIGH << 2))

#define WOW_GPIO_HIGH_TO_LOW_PARAMETER                                         \
	((WOW_GPIO_OUTPUT_ENABLE << 0) | (WOW_GPIO_OUTPUT_LEVEL_HIGH << 1) |   \
	 (WOW_GPIO_WAKEUP_LEVEL_LOW << 2));

typedef enum { WOW_PKT_TO_HOST, WOW_PKT_TO_ANDES } WOW_PKT_FLOW_T;
...
typedef enum { WOW_ENABLE = 1, WOW_TRAFFIC = 3, WOW_WAKEUP = 4 } WOW_FEATURE_T;
...
enum { WOW_MAGIC_PKT, WOW_BITMAP, WOW_IPV4_TCP_SYNC, WOW_IPV6_TCP_SYNC };
...
#define DEFLAUT_PARTIAL_SCAN_BREAK_TIME                                        \
	4 /* Period of partial scaning: unit: 100ms */
...
typedef struct _CFG80211_CONTROL {
	...
	UCHAR Cfg_pending_Ssid[MAX_LEN_OF_SSID +
			       1]; /* Record the ssid, When ScanTable Full */
...
#define QLOAD_DOES_ALARM_OCCUR(pAd)                                            \
	(HcGetQloadCtrl(pAd)->FlgQloadAlarmIsSuspended == TRUE)
...
struct _RTMP_ADAPTER {
	...
	defined(OFFCHANNEL_SCAN_FEATURE)
	...
	NDIS_SPIN_LOCK
	McuCmdLock; /*MCU Command Queue spinlock for RT3090/3592/3390/3593/5390/5392/5592/3290 */
#endif /* RTMP_MAC_PCI */
...
#define CAL_TXDPD_SIZE (CAL_TXDPD_PERCHAN_SIZE * 14)
	...
	Antenna; /* Since ANtenna definition is different for a & g. We need to save it for future reference. */
#ifdef DBDC_MODE
	...
	UCHAR BbpWriteLatch
		[MAX_BBP_ID +
		 1]; /* record last BBP register value written via BBP_IO_WRITE/BBP_IO_WRITE_VY_REG_ID */
	UCHAR Bbp94;
	...
	RTMP_RF_REGS
	LatchRfRegs; /* latch th latest RF programming value since RF IC doesn't support READ */
	...
	/* TxPower control						   */
	/* ---------------------------- */
	CHANNEL_TX_POWER TxPower
		[MAX_NUM_OF_CHANNELS]; /* Store Tx power value for all channels. */
	CHANNEL_TX_POWER ChannelList
		[MAX_NUM_OF_CHANNELS]; /* list all supported channels for site survey */
...
#ifdef DOT11_VHT_AC
	ULONG Tx80MPwrCfgABand
		[MAX_TXPOWER_ARRAY_SIZE]; /* Per-rate Tx power control for VHT BW80 (5GHz only) */
	...
	UCHAR TssiPlusBoundaryA
		[2]
		[8]; /* Tssi boundary for increase Tx power to compensate. [Group][Boundary Index]*/
	UCHAR TssiMinusBoundaryA
		[2]
		[8]; /* Tssi boundary for decrease Tx power to compensate. [Group][Boundary Index]*/
	...
	UCHAR TssiPlusBoundaryG
		[8]; /* Tssi boundary for increase Tx power to compensate. */
	UCHAR TssiMinusBoundaryG
		[8]; /* Tssi boundary for decrease Tx power to compensate. */
	...
	signed char BGRssiOffset
		[4]; /* Store B/G RSSI #0/1/2 Offset value on EEPROM 0x46h */
	signed char ARssiOffset
		[4]; /* Store A RSSI 0/1/2 Offset value on EEPROM 0x4Ah */
	...
	NDIS_MEDIA_STATE
	IndicateMediaState; /* Base on Indication state, default is NdisMediaStateDisConnected */
	...
	*/
...
#ifdef MT_MAC
	UCHAR ExtendMBssAddr[EXTEND_MBSS_MAC_MAX]
			    [MAC_ADDR_LEN]; /* User defined MAC address for MBSSID*/
	...
	CIPHER_KEY SharedKey[HW_BEACON_MAX_NUM + MAX_P2P_NUM]
...
#define TIME_ONE_SECOND (1000000 / TIME_BASE)
	...
	(RTMP_ADAPTER *pAd, PNDIS_PACKET pPkt, UINT infIdx);
#endif /* MAT_SUPPORT */
	...
	NDIS_SPIN_LOCK
	smartAntLock; /* lock used to handle synchronization of smart antenna algorithms. */
#endif /* SMART_ANTENNA */
...
#define MAX_FW_DUMP_SIZE 1024000
	...
	ULONG ConInPrgress[(MAX_LEN_OF_MAC_TABLE) /
			   (sizeof(ULONG) * NUM_OF_BITS_IN_BYTE)];
#else
	ULONG ConInPrgress[(MAX_LEN_OF_MAC_TABLE) /
				   (sizeof(ULONG) * NUM_OF_BITS_IN_BYTE) +
			   1];
...
typedef struct _RX_BLK {
	UCHAR hw_rx_info
		[RXD_SIZE]; /* include "RXD_STRUC RxD" and "RXINFO_STRUC rx_info " */
	RXINFO_STRUC *
		pRxInfo; /* for RLT, in head of frame buffer, for RTMP, in hw_rx_info[RXINFO_OFFSET] */
...
typedef struct _TX_BLK {
	...
	struct wifi_dev *wdev;
	MAC_TABLE_ENTRY *
		pMacEntry; /* NULL: packet with 802.11 RA field is multicast/broadcast address */
	TX_BLK_FLAGS Flags;
...
#define TX_BLK_TEST_FLAG(_pTxBlk, _flag)                                       \
	(((_pTxBlk->Flags & _flag) == _flag) ? 1 : 0)
...
static inline BOOLEAN RTMPIsTcpDataPkt(IN PNDIS_PACKET pPacket)
{
	UINT32 pktlen;
...
static inline BOOLEAN RTMPIsTcpAckPkt(IN PNDIS_PACKET pPacket)
{
	UINT32 pktlen;
...
static inline VOID mt_rmac_base_info_endian_change(IN RTMP_ADAPTER *pAd,
						   IN UCHAR *rxinfo)
{
	int i;
...
/*
	...
	========================================================================
*/
static inline VOID RTMPWIEndianChange(IN RTMP_ADAPTER *pAd, IN UCHAR *pData,
				      IN ULONG DescriptorType)
{
	...
	if (DescriptorType == TYPE_TXWI) {
		*((UINT32 *)(pData)) =
			SWAP32(*((UINT32 *)(pData))); /* Byte 0~3 */
		*((UINT32 *)(pData + 4)) =
			SWAP32(*((UINT32 *)(pData + 4))); /* Byte 4~7 */

		if (size > 16)
			*((UINT32 *)(pData + 16)) = SWAP32(
				*((UINT32 *)(pData + 16))); /* Byte 16~19 */
	} else {
		for (i = 0; i < size / 4; i++)
			*(((UINT32 *)pData) + i) =
				SWAP32(*(((UINT32 *)pData) + i));
	}
}
...
/*
	...
	========================================================================
*/
static inline VOID MTMacInfoEndianChange(IN RTMP_ADAPTER *pAd, IN UCHAR *pData,
					 IN ULONG DescriptorType,
					 IN UINT16 Length)
{
	int i;
	...
	if (DescriptorType == TYPE_TMACINFO) {
		...
		for (i = 1; i < Length / 4;
		     i++) /* i from 1, due to 1st DW had endia change already, so skip it here. */
			*(((UINT32 *)pData) + i) =
				SWAP32(*(((UINT32 *)pData) + i));
	}
}
...
#ifdef RTMP_MAC_PCI
static inline VOID WriteBackToDescriptor(IN UCHAR *Dest, IN UCHAR *Src,
...
static inline VOID RTMPDescriptorEndianChange(UCHAR *pData, ULONG DescType)
{
	...
	*((UINT32 *)(pData + 8)) =
		SWAP32(*((UINT32 *)(pData + 8))); /* Byte 8~11 */
	/*new txd & rxd just have 3 word*/
#ifndef MT7615
	*((UINT32 *)(pData + 12)) =
		SWAP32(*((UINT32 *)(pData + 12))); /* Byte 12~15 */
#endif
	*((UINT32 *)(pData + 4)) = SWAP32(
		*((UINT32 *)(pData +
...
/*
	...
	========================================================================
*/
static inline VOID RTMPFrameEndianChange(IN RTMP_ADAPTER *pAd, IN PUCHAR pData,
		...
		case SUBTYPE_BLOCK_ACK_REQ: {
			...
			*(USHORT *)(&pBAReq->BARControl) =
			...
			pBAReq->BAStartingSeq.word =
				SWAP16(pBAReq->BAStartingSeq.word);
		} break;

		case SUBTYPE_BLOCK_ACK:
			/* For Block Ack packet, the HT_CONTROL field is in the same offset with Addr3 */
			*(UINT32 *)(&pFrame->Addr3[0]) =
				SWAP32(*(UINT32 *)(&pFrame->Addr3[0]));
			break;

		case SUBTYPE_ACK:
			/*For ACK packet, the HT_CONTROL field is in the same offset with Addr2 */
			*(UINT32 *)(&pFrame->Addr2[0]) =
				SWAP32(*(UINT32 *)(&pFrame->Addr2[0]));
			break;
		}
	} else
...
static inline VOID RTMPEndianChange(IN UCHAR *pData, IN UINT size)
{
	int i;
...
NDIS_STATUS RTMPSetPreProfileParameters(RTMP_ADAPTER *pAd,
					RTMP_STRING *pBuffer);

INT RTMPGetKeyParameter(IN RTMP_STRING *key, OUT RTMP_STRING *dest,
			IN INT destsize, IN RTMP_STRING *buffer,
			IN BOOLEAN bTrimSpace);

INT RTMPSetKeyParameter(IN RTMP_STRING *key, OUT CHAR *value, IN INT destsize,
			IN RTMP_STRING *buffer, IN BOOLEAN bTrimSpace);

INT RTMPAddKeyParameter(IN RTMP_STRING *key, OUT CHAR *value, IN INT destsize,
			IN RTMP_STRING *buffer);

INT RTMPGetKeyParameterWithOffset(IN RTMP_STRING *key, OUT RTMP_STRING *dest,
				  OUT USHORT *end_offset, IN INT destsize,
				  IN RTMP_STRING *buffer,
				  IN BOOLEAN bTrimSpace);

#ifdef MIN_PHY_RATE_SUPPORT
VOID RTMPMinPhyDataRateCfg(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *Buffer);

VOID RTMPMinPhyBeaconRateCfg(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *Buffer);

VOID RTMPMinPhyMgmtRateCfg(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *Buffer);

VOID RTMPMinPhyBcMcRateCfg(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *Buffer);

VOID RTMPLimitClientSupportRateCfg(IN RTMP_ADAPTER *pAd,
				   IN RTMP_STRING *Buffer);

VOID RTMPDisableCCKRateCfg(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *Buffer);
#endif /* MIN_PHY_RATE_SUPPORT */

#ifdef WSC_INCLUDED
VOID rtmp_read_wsc_user_parms_from_file(IN RTMP_ADAPTER *pAd, IN char *tmpbuf,
					IN char *buffer);
#endif /*WSC_INCLUDED*/

#ifdef DSCP_QOS_MAP_SUPPORT
VOID dscp_qosmap_update_sta_mapping_to_cr4(IN RTMP_ADAPTER *pAd,
					   IN struct _MAC_TABLE_ENTRY *pEntry,
					   IN UINT8 PoolID);

VOID DscpQosMapInit(IN RTMP_ADAPTER *pAd);
#endif
...
#ifdef VOW_SUPPORT
#ifdef CONFIG_AP_SUPPORT
void rtmp_read_vow_parms_from_file(IN PRTMP_ADAPTER pAd, char *tmpbuf,
				   char *buffer);
#endif /* CONFIG_AP_SUPPORT */
#endif /* VOW_SUPPORT */

#ifdef RED_SUPPORT
void rtmp_read_red_parms_from_file(IN PRTMP_ADAPTER pAd, char *tmpbuf,
				   char *buffer);
#endif /* RED_SUPPORT */
#ifdef FQ_SCH_SUPPORT
void rtmp_read_fq_parms_from_file(IN PRTMP_ADAPTER pAd, char *tmpbuf,
				  char *buffer);
#endif /* FQ_SCH_SUPPORT */

void rtmp_read_cp_parms_from_file(IN PRTMP_ADAPTER pAd, char *tmpbuf,
				  char *buffer);

VOID cp_support_is_enabled(PRTMP_ADAPTER pAd);

INT set_cp_support_en(IN PRTMP_ADAPTER pAd, IN RTMP_STRING *arg);
...
UCHAR GetSkuChannelBasePwr(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
			   UCHAR channel);
VOID UpdateSkuRatePwr(RTMP_ADAPTER *pAd, UCHAR ch, UCHAR bw, CHAR base_pwr);
#endif /* SINGLE_SKU_V2 */

#ifdef RTMP_RF_RW_SUPPORT
VOID RTMP_ReadRF(IN RTMP_ADAPTER *pAd, IN UCHAR RegID, OUT PUCHAR pValue1,
		 OUT PUCHAR pValue2, IN UCHAR BitMask);

VOID RTMP_WriteRF(IN RTMP_ADAPTER *pAd, IN UCHAR RegID, IN UCHAR Value,
		  IN UCHAR BitMask);

NDIS_STATUS RT30xxWriteRFRegister(IN RTMP_ADAPTER *pAd, IN UCHAR regID,
				  IN UCHAR value);

NDIS_STATUS RT30xxReadRFRegister(IN RTMP_ADAPTER *pAd, IN UCHAR regID,
				 IN PUCHAR pValue);

NDIS_STATUS RT635xWriteRFRegister(IN RTMP_ADAPTER *pAd, IN UCHAR bank,
				  IN UCHAR regID, IN UCHAR value);

NDIS_STATUS RT635xReadRFRegister(IN RTMP_ADAPTER *pAd, IN UCHAR bank,
				 IN UCHAR regID, IN PUCHAR pValue);

BOOLEAN RTMPAdjustFrequencyOffset(IN RTMP_ADAPTER *pAd,
				  INOUT PUCHAR pRefFreqOffset);
#endif /* RTMP_RF_RW_SUPPORT */

BOOLEAN RTMPCheckPhyMode(IN RTMP_ADAPTER *pAd, IN UINT8 BandSupported,
...
VOID NicGetTxRawCounters(IN RTMP_ADAPTER *pAd, IN TX_STA_CNT0_STRUC *pStaTxCnt0,
...
VOID RTMPInitTimer(IN RTMP_ADAPTER *pAd, IN RALINK_TIMER_STRUCT *pTimer,
...
/* */
VOID ActHeaderInit(IN RTMP_ADAPTER *pAd, IN OUT HEADER_802_11 *pHdr80211,
		   IN UCHAR *da, IN UCHAR *sa, IN UCHAR *bssid);

VOID ActionStateMachineInit(IN RTMP_ADAPTER *pAd, IN STATE_MACHINE *S,
...
#ifdef DOT11_N_SUPPORT
VOID RECBATimerTimeout(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
...
VOID RTMP_11N_D3_TimerRelease(IN RTMP_ADAPTER *pAd);

VOID SendBSS2040CoexistMgmtAction(IN RTMP_ADAPTER *pAd, IN UCHAR Wcid,
				  IN UCHAR apidx, IN UCHAR InfoReq);

VOID SendNotifyBWActionFrame(IN RTMP_ADAPTER *pAd, IN UCHAR Wcid,
			     IN UCHAR apidx);

BOOLEAN ChannelSwitchSanityCheck(IN RTMP_ADAPTER *pAd, IN UCHAR Wcid,
				 IN UCHAR NewChannel, IN UCHAR Secondary);

VOID ChannelSwitchAction(IN RTMP_ADAPTER *pAd, IN UCHAR Wcid, IN UCHAR Channel,
			 IN UCHAR Secondary);

ULONG BuildIntolerantChannelRep(IN RTMP_ADAPTER *pAd, IN PUCHAR pDest);

VOID Update2040CoexistFrameAndNotify(IN RTMP_ADAPTER *pAd, IN UCHAR Wcid,
				     IN BOOLEAN bAddIntolerantCha);

VOID Send2040CoexistAction(IN RTMP_ADAPTER *pAd, IN UCHAR Wcid,
			   IN BOOLEAN bAddIntolerantCha);
...
#endif /* DOT11_N_SUPPORT */
UCHAR get_regulatory_class(RTMP_ADAPTER *pAd, UCHAR Channel, UCHAR PhyMode,
			   struct wifi_dev *wdev);

UCHAR get_channel_set_num(UCHAR *ChannelSet);

PUCHAR get_channelset_by_reg_class(IN RTMP_ADAPTER *pAd,
				   IN UINT8 RegulatoryClass, IN UCHAR PhyMode);

INT Set_Reg_Domain_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);

VOID BarHeaderInit(IN RTMP_ADAPTER *pAd, IN OUT PFRAME_BAR pCntlBar,
		   IN PUCHAR pDA, IN PUCHAR pSA);

VOID InsertActField(IN RTMP_ADAPTER *pAd, OUT PUCHAR pFrameBuf,
		    OUT PULONG pFrameLen, IN UINT8 Category, IN UINT8 ActCode);

BOOLEAN QosBADataParse(IN RTMP_ADAPTER *pAd, IN BOOLEAN bAMSDU,
		       IN PUCHAR p8023Header, IN UCHAR WCID, IN UCHAR TID,
		       IN USHORT Sequence, IN UCHAR DataOffset,
		       IN USHORT Datasize, IN UINT CurRxIndex);

BOOLEAN bar_process(RTMP_ADAPTER *pAd, ULONG Wcid, ULONG MsgLen,
		    PFRAME_BA_REQ pMsg);
...
INT32 wdev_init(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
		enum WDEV_TYPE wdev_type, PNET_DEV if_dev, INT8 func_idx,
		VOID *func_dev, VOID *sys_handle);
...
INT wdev_tx_pkts(NDIS_HANDLE dev_hnd, PPNDIS_PACKET pkt_list, UINT pkt_cnt,
		 struct wifi_dev *wdev);
UCHAR get_frag_num(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
		   PNDIS_PACKET pPacket);
...
VOID BssInfoArgumentLink(struct _RTMP_ADAPTER *ad, struct wifi_dev *wdev,
			 struct _BSS_INFO_ARGUMENT_T *bssinfo);
VOID wdev_if_up_down(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
		     BOOLEAN if_up_down_state);
...
#ifdef BW_VENDOR10_CUSTOM_FEATURE
#ifdef DOT11_N_SUPPORT
void wdev_sync_ht_bw(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
		     ADD_HTINFO *add_ht_info);
#endif
#ifdef DOT11_VHT_AC
void wdev_sync_vht_bw(struct _RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
		      UCHAR bw, UINT8 channel);
#endif
BOOLEAN IS_SYNC_BW_POLICY_VALID(struct _RTMP_ADAPTER *pAd, BOOLEAN isHTPolicy,
				UCHAR policy);
#endif
...
INT rtmp_IpAssembleHandle(RTMP_ADAPTER *pAd, STA_TR_ENTRY *pTrEntry,
			  PNDIS_PACKET pPacket, UCHAR queIdx,
			  PACKET_INFO packetInfo);
#endif

VOID ba_ori_session_start(IN RTMP_ADAPTER *pAd, IN STA_TR_ENTRY *tr_entry,
...
VOID mt_detect_wmm_traffic(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
			   UCHAR UserPriority, UCHAR FlgIsOutput);
VOID rx_802_3_data_frm_announce(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry,
				RX_BLK *pRxBlk, struct wifi_dev *wdev);

VOID rx_data_frm_announce(IN RTMP_ADAPTER *pAd, IN MAC_TABLE_ENTRY *pEntry,
			  IN RX_BLK *pRxBlk, IN struct wifi_dev *wdev);

INT sta_rx_pkt_allow(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, RX_BLK *pRxBlk);
INT rx_chk_duplicate_frame(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk,
			   struct wifi_dev *wdev);
VOID mt_dynamic_wmm_be_tx_op(IN RTMP_ADAPTER *pAd, IN ULONG nonBEpackets);

NDIS_STATUS RTMPFreeTXDRequest(IN RTMP_ADAPTER *pAd, IN UCHAR RingType,
			       IN UCHAR NumberRequired, IN PUCHAR FreeNumberIs);

NDIS_STATUS MlmeHardTransmit(IN RTMP_ADAPTER *pAd, IN UCHAR QueIdx,
			     IN PNDIS_PACKET pPacket, IN BOOLEAN FlgDataQForce,
			     IN BOOLEAN FlgIsLocked, IN BOOLEAN FlgIsCheckPS);

NDIS_STATUS MlmeHardTransmitMgmtRing(IN RTMP_ADAPTER *pAd, IN UCHAR QueIdx,
				     IN PNDIS_PACKET pPacket);

#ifdef RTMP_MAC_PCI
NDIS_STATUS MlmeHardTransmitTxRing(IN RTMP_ADAPTER *pAd, IN UCHAR QueIdx,
				   IN PNDIS_PACKET pPacket);

NDIS_STATUS MlmeDataHardTransmit(IN RTMP_ADAPTER *pAd, IN UCHAR QueIdx,
				 IN PNDIS_PACKET pPacket);
...
VOID mtd_write_tmac_info_fixed_rate(IN RTMP_ADAPTER *pAd, IN UCHAR *tmac_info,
				    IN MAC_TX_INFO *info,
				    IN HTTRANSMIT_SETTING *pTransmit);

VOID mt_write_tmac_info_fixed_rate(IN RTMP_ADAPTER *pAd, IN UCHAR *tmac_info,
				   IN MAC_TX_INFO *info,
				   IN HTTRANSMIT_SETTING *pTransmit);

VOID rtmp_write_tmac_info_fixed_rate(IN RTMP_ADAPTER *pAd, IN UCHAR *tmac_info,
				     IN MAC_TX_INFO *info,
				     IN HTTRANSMIT_SETTING *pTransmit);
...
#ifdef MT_MAC
VOID mt_write_tmac_info_beacon(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
			       UCHAR *tmac_buf,
			       HTTRANSMIT_SETTING *BeaconTransmit,
			       ULONG frmLen);
#endif /* MT_MAC */

VOID RTMPSuspendMsduTransmission(IN RTMP_ADAPTER *pAd, struct wifi_dev *wdev);

VOID RTMPResumeMsduTransmission(IN RTMP_ADAPTER *pAd, struct wifi_dev *wdev);

NDIS_STATUS MiniportMMRequest(IN RTMP_ADAPTER *pAd, IN UCHAR QueIdx,
			      IN UCHAR *pData, IN UINT Length);

VOID LowPowerDebug(PRTMP_ADAPTER pAd, PSTA_ADMIN_CONFIG pStaCfg);

VOID RTMPSendNullFrame(IN RTMP_ADAPTER *pAd, IN PSTA_ADMIN_CONFIG pStaCfg,
...
BOOLEAN RTMPFreeTXDUponTxDmaDone(IN RTMP_ADAPTER *pAd, IN UCHAR QueIdx,
				 IN UCHAR RingIdx, BOOLEAN ForceFree);

#ifdef MT_MAC
UINT32 pkt_alloc_fail_handle(RTMP_ADAPTER *ad, PNDIS_PACKET rx_packet);
UINT32 mt_rx_pkt_process(RTMP_ADAPTER *, UINT8 hif_idx, RX_BLK *pRxBlk,
			 PNDIS_PACKET pRxPacket);
#endif /* MT_MAC */

#if defined(CONFIG_WIFI_PKT_FWD) || defined(CONFIG_WIFI_PKT_FWD_MODULE)
BOOLEAN is_looping_packet(IN RTMP_ADAPTER *pAd, IN NDIS_PACKET *pPacket);

VOID set_wf_fwd_cb(IN RTMP_ADAPTER *pAd, IN PNDIS_PACKET pPacket,
...
BOOLEAN RTMPGetUserPriority(IN RTMP_ADAPTER *pAd, IN PNDIS_PACKET pPacket,
			    IN struct wifi_dev *wdev, OUT UCHAR *pUserPriority,
			    OUT UCHAR *pQueIdx);

#endif
#ifdef CUSTOMER_DCC_FEATURE

VOID APGetStreamingStatus(IN PRTMP_ADAPTER pAd, IN UCHAR QueIdx);
#endif

BOOLEAN RTMPCheckEtherType(IN RTMP_ADAPTER *pAd, IN PNDIS_PACKET pPacket,
			   IN STA_TR_ENTRY *tr_entry, IN struct wifi_dev *wdev);
...
VOID AsicPercentageDeltaPower(IN PRTMP_ADAPTER pAd, IN CHAR Rssi,
			      INOUT PCHAR pDeltaPwr,
			      INOUT PCHAR pDeltaPowerByBbpR1);

VOID AsicCompensatePowerViaBBP(IN RTMP_ADAPTER *pAd,
...
#ifdef RTMP_RBUS_SUPPORT
int RtmpAsicSendCommandToSwMcu(IN RTMP_ADAPTER *pAd, IN UCHAR Command,
...
INT Set_StreamMode_Proc(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);

INT Set_StreamModeMac_Proc(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);

INT Set_StreamModeMCS_Proc(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);
#endif /* STREAM_MODE_SUPPORT */

VOID MacAddrRandomBssid(IN RTMP_ADAPTER *pAd, OUT PUCHAR pAddr);

VOID MgtMacHeaderInit(IN RTMP_ADAPTER *pAd, INOUT HEADER_802_11 *pHdr80211,
		      IN UCHAR SubType, IN UCHAR ToDs, IN UCHAR *pDA,
		      IN UCHAR *pSA, IN UCHAR *pBssid);

VOID MgtMacHeaderInitExt(IN RTMP_ADAPTER *pAd, IN OUT PHEADER_802_11 pHdr80211,
			 IN UCHAR SubType, IN UCHAR ToDs, IN PUCHAR pDA,
			 IN PUCHAR pSA, IN PUCHAR pBssid);

VOID MlmeRadioOff(IN RTMP_ADAPTER *pAd, struct wifi_dev *wdev);

VOID MlmeRadioOn(IN RTMP_ADAPTER *pAd, struct wifi_dev *wdev);

VOID MlmeLpEnter(IN RTMP_ADAPTER *pAd);

VOID MlmeLpExit(IN RTMP_ADAPTER *pAd);

VOID BssTableInit(IN BSS_TABLE *Tab);

#if defined(DBDC_MODE) && defined(DOT11K_RRM_SUPPORT)
VOID BssTableInitByBand(IN BSS_TABLE *Tab, IN UCHAR Band);
...
ULONG BssSsidTableSearchBySSID(IN BSS_TABLE *Tab, IN PUCHAR pSsid,
			       IN UCHAR SsidLen);

VOID BssTableDeleteEntry(IN OUT PBSS_TABLE pTab, IN PUCHAR pBssid,
			 IN UCHAR Channel);

ULONG BssTableSetEntry(IN RTMP_ADAPTER *pAd, OUT BSS_TABLE *Tab,
		       IN BCN_IE_LIST *ie_list, IN CHAR Rssi,
		       IN USHORT LengthVIE, IN PNDIS_802_11_VARIABLE_IEs pVIE
#if defined(CUSTOMER_DCC_FEATURE) || defined(CONFIG_MAP_SUPPORT) ||            \
	defined(NEIGHBORING_AP_STAT)
		       ,
		       IN UCHAR *Snr0, IN CHAR *rssi
#endif
);

#ifdef DOT11_N_SUPPORT
VOID BATableInsertEntry(IN RTMP_ADAPTER *pAd, IN USHORT Aid,
			IN USHORT TimeOutValue, IN USHORT StartingSeq,
			IN UCHAR TID, IN UCHAR BAWinSize,
...
INT TriEventTableSetEntry(IN RTMP_ADAPTER *pAd, OUT TRIGGER_EVENT_TAB *Tab,
			  IN PUCHAR pBssid, IN HT_CAPABILITY_IE *pHtCapability,
			  IN UCHAR HtCapabilityLen, IN UCHAR RegClass,
			  IN UCHAR ChannelNo);

#endif /* DOT11N_DRAFT3 */
#endif /* DOT11_N_SUPPORT */

VOID BssTableSsidSort(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev,
...
NDIS_STATUS MlmeQueueInit(IN RTMP_ADAPTER *pAd);

VOID MlmeQueueDestroy(IN struct _MLME_STRUCT *mlme);

#ifdef MLME_MULTI_QUEUE_SUPPORT
VOID *MlmeQueueSelByMach(IN RTMP_ADAPTER *pAd, IN ULONG Machine,
			 IN ULONG MsgType, IN VOID *pHeader);
#endif /*MLME_MULTI_QUEUE_SUPPORT*/

#ifdef CONFIG_AP_SUPPORT
BOOLEAN MlmeEnqueue(IN RTMP_ADAPTER *pAd, IN ULONG Machine, IN ULONG MsgType,
		    IN ULONG MsgLen, IN VOID *Msg, IN ULONG Priv);
#endif

BOOLEAN MlmeEnqueueWithWdev(IN RTMP_ADAPTER *pAd, IN ULONG Machine,
			    IN ULONG MsgType, IN ULONG MsgLen, IN VOID *Msg,
			    IN ULONG Priv, IN struct wifi_dev *wdev);

BOOLEAN MlmeEnqueueForRecv(IN RTMP_ADAPTER *pAd, IN ULONG Wcid,
			   IN struct raw_rssi_info *rssi_info, IN ULONG MsgLen,
			   IN PVOID Msg, IN UCHAR OpMode,
			   IN struct wifi_dev *wdev, IN UCHAR RxPhyMode);

#ifdef WSC_INCLUDED
BOOLEAN MlmeEnqueueForWsc(IN RTMP_ADAPTER *pAd, IN ULONG eventID,
			  IN LONG senderID, IN ULONG Machine, IN ULONG MsgType,
			  IN ULONG MsgLen, IN VOID *Msg,
			  IN struct wifi_dev *wdev);
#endif /* WSC_INCLUDED */

BOOLEAN MlmeDequeue(IN MLME_QUEUE *Queue, OUT MLME_QUEUE_ELEM **Elem);

VOID MlmeRestartStateMachine(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev);

BOOLEAN MlmeQueueEmpty(IN MLME_QUEUE *Queue);

BOOLEAN MlmeQueueFull(IN MLME_QUEUE *Queue, IN UCHAR SendId);

BOOLEAN MsgTypeSubst(IN RTMP_ADAPTER *pAd, IN PFRAME_802_11 pFrame,
		     OUT INT *Machine, OUT INT *MsgType);

VOID StateMachineInit(IN STATE_MACHINE *Sm, IN STATE_MACHINE_FUNC Trans[],
		      IN ULONG StNr, IN ULONG MsgNr,
		      IN STATE_MACHINE_FUNC DefFunc, IN ULONG InitState,
		      IN ULONG Base);

VOID StateMachineSetAction(IN STATE_MACHINE *S, IN ULONG St, ULONG Msg,
			   IN STATE_MACHINE_FUNC F);

VOID StateMachinePerformAction(IN RTMP_ADAPTER *pAd, IN STATE_MACHINE *S,
			       IN MLME_QUEUE_ELEM *Elem, IN ULONG CurrState);

VOID Drop(IN RTMP_ADAPTER *pAd, IN MLME_QUEUE_ELEM *Elem);

VOID AssocStateMachineInit(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev,
			   IN STATE_MACHINE *Sm,
			   OUT STATE_MACHINE_FUNC Trans[]);

VOID ReassocTimeout(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
		    IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

VOID AssocTimeout(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
		  IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

VOID DisassocTimeout(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
		     IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);
...
VOID InvalidStateWhenDisassociate(IN RTMP_ADAPTER *pAd,
				  IN MLME_QUEUE_ELEM *Elem);

VOID ComposePsPoll(IN RTMP_ADAPTER *pAd, IN PPSPOLL_FRAME pPsPollFrame,
		   IN USHORT Aid, IN UCHAR *pBssid, IN UCHAR *pTa);

VOID ComposeNullFrame(RTMP_ADAPTER *pAd, PHEADER_802_11 pNullFrame,
		      UCHAR *pAddr1, UCHAR *pAddr2, UCHAR *pAddr3);

VOID AssocPostProc(IN RTMP_ADAPTER *pAd, IN PUCHAR pAddr2,
		   IN USHORT CapabilityInfo, IN USHORT Aid, IN UCHAR SupRate[],
		   IN UCHAR SupRateLen, IN UCHAR ExtRate[], IN UCHAR ExtRateLen,
		   IN PEDCA_PARM pEdcaParm, struct _IE_lists *ie_list,
		   IN HT_CAPABILITY_IE *pHtCapability, IN UCHAR HtCapabilityLen,
		   IN ADD_HT_INFO_IE *pAddHtInfo, IN MAC_TABLE_ENTRY *pEntry);

VOID AuthStateMachineInit(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev,
			  IN PSTATE_MACHINE sm, OUT STATE_MACHINE_FUNC Trans[]);

VOID AuthTimeout(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
		 IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);
...
VOID Cls2errAction(IN struct wifi_dev *wdev, IN PUCHAR pAddr);
...
VOID AuthRspStateMachineInit(IN RTMP_ADAPTER *pAd, IN PSTATE_MACHINE Sm,
...
VOID PeerAuthSimpleRspGenAndSend(IN RTMP_ADAPTER *pAd,
				 ...
				 IN USHORT Seq, IN USHORT Reason,
				 IN USHORT Status);

BOOLEAN PeerProbeReqSanity(IN RTMP_ADAPTER *pAd, IN VOID *Msg, IN ULONG MsgLen,
			   OUT PEER_PROBE_REQ_PARAM *Param);

/*======================================== */

VOID SyncStateMachineInit(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev,
			  IN STATE_MACHINE *Sm, OUT STATE_MACHINE_FUNC Trans[]);

VOID BeaconTimeout(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
		   IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);
...
VOID MlmeCntlInit(IN PRTMP_ADAPTER pAd, IN struct wifi_dev *wdev);

VOID MlmeCntlMachinePerformAction(IN RTMP_ADAPTER *pAd, IN STATE_MACHINE *S,
				  IN MLME_QUEUE_ELEM *Elem);
...
VOID LinkUp(IN RTMP_ADAPTER *pAd, IN UCHAR BssType, IN struct wifi_dev *wdev);

VOID LinkDown(IN RTMP_ADAPTER *pAd, IN BOOLEAN IsReqFromAP,
	      IN struct wifi_dev *wdev);

VOID IterateOnBssTab(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev);

VOID IterateOnBssTab2(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev);

VOID JoinParmFill(IN RTMP_ADAPTER *pAd, IN OUT MLME_JOIN_REQ_STRUCT *JoinReq,
		  IN ULONG BssIdx);

VOID AssocParmFill(IN RTMP_ADAPTER *pAd, IN OUT MLME_ASSOC_REQ_STRUCT *AssocReq,
		   IN PUCHAR pAddr, IN USHORT CapabilityInfo, IN ULONG Timeout,
		   IN USHORT ListenIntv);

VOID ScanParmFill(IN RTMP_ADAPTER *pAd, IN OUT MLME_SCAN_REQ_STRUCT *ScanReq,
		  IN RTMP_STRING Ssid[], IN UCHAR SsidLen, IN UCHAR BssType,
		  IN UCHAR ScanType);
...
VOID StartParmFill(IN RTMP_ADAPTER *pAd, IN OUT MLME_START_REQ_STRUCT *StartReq,
		   IN CHAR Ssid[], IN UCHAR SsidLen);

VOID AuthParmFill(IN RTMP_ADAPTER *pAd, IN OUT MLME_AUTH_REQ_STRUCT *AuthReq,
		  IN PUCHAR pAddr, IN USHORT Alg);

VOID EnqueuePsPoll(IN RTMP_ADAPTER *pAd, PSTA_ADMIN_CONFIG pStaCfg);
...
#ifdef VENDOR10_CUSTOM_RSSI_FEATURE
#define IS_VENDOR10_RSSI_VALID(_wdev) (_wdev->isRssiEnbl == TRUE)
#define SET_VENDOR10_RSSI_VALID(_wdev, enable) (_wdev->isRssiEnbl = enable)

VOID Vendor10RssiUpdate(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry,
			IN BOOLEAN isBcn, IN INT RealRssi);
#endif
...
BOOLEAN MlmeScanReqSanity(IN RTMP_ADAPTER *pAd, IN VOID *Msg, IN ULONG MsgLen,
			  OUT UCHAR *BssType, OUT CHAR ssid[],
			  OUT UCHAR *SsidLen, OUT UCHAR *ScanType);

BOOLEAN PeerBeaconAndProbeRspSanity_Old(
	IN RTMP_ADAPTER *pAd, IN VOID *Msg, IN ULONG MsgLen,
	IN UCHAR MsgChannel, OUT PUCHAR pAddr2, OUT PUCHAR pBssid,
	OUT CHAR Ssid[], OUT UCHAR *pSsidLen, OUT UCHAR *pBssType,
	OUT USHORT *pBeaconPeriod, OUT UCHAR *pChannel, OUT UCHAR *pNewChannel,
	OUT LARGE_INTEGER *pTimestamp, OUT CF_PARM *pCfParm,
	OUT USHORT *pAtimWin, OUT USHORT *pCapabilityInfo, OUT UCHAR *pErp,
	OUT UCHAR *pDtimCount, OUT UCHAR *pDtimPeriod, OUT UCHAR *pBcastFlag,
	OUT UCHAR *pMessageToMe, OUT UCHAR SupRate[], OUT UCHAR *pSupRateLen,
	OUT UCHAR ExtRate[], OUT UCHAR *pExtRateLen, OUT UCHAR *pCkipFlag,
	OUT UCHAR *pAironetCellPowerLimit, OUT PEDCA_PARM pEdcaParm,
	OUT PQBSS_LOAD_PARM pQbssLoad, OUT PQOS_CAPABILITY_PARM pQosCapability,
	OUT ULONG *pRalinkIe, OUT UCHAR *pHtCapabilityLen,
	OUT HT_CAPABILITY_IE *pHtCapability,
	OUT EXT_CAP_INFO_ELEMENT *pExtCapInfo, OUT UCHAR *AddHtInfoLen,
	OUT ADD_HT_INFO_IE *AddHtInfo, OUT UCHAR *NewExtChannel,
	OUT USHORT *LengthVIE, OUT PNDIS_802_11_VARIABLE_IEs pVIE);

BOOLEAN PeerBeaconAndProbeRspSanity(IN RTMP_ADAPTER *pAd,
				    IN struct wifi_dev *wdev, IN VOID *Msg,
				    IN ULONG MsgLen, IN UCHAR MsgChannel,
...
#ifdef DOT11_N_SUPPORT
#ifdef DOT11N_DRAFT3
BOOLEAN PeerBeaconAndProbeRspSanity2(IN RTMP_ADAPTER *pAd, IN VOID *Msg,
				     IN ULONG MsgLen,
...
BOOLEAN PeerAddBAReqActionSanity(IN RTMP_ADAPTER *pAd, IN VOID *pMsg,
				 IN ULONG MsgLen, OUT PUCHAR pAddr2);

BOOLEAN PeerAddBARspActionSanity(IN RTMP_ADAPTER *pAd, IN VOID *pMsg,
				 IN ULONG MsgLen);

BOOLEAN PeerDelBAActionSanity(IN RTMP_ADAPTER *pAd, IN UCHAR Wcid,
			      IN VOID *pMsg, IN ULONG MsgLen);

BOOLEAN MlmeAssocReqSanity(IN RTMP_ADAPTER *pAd, IN VOID *Msg, IN ULONG MsgLen,
			   OUT PUCHAR pApAddr, OUT USHORT *CapabilityInfo,
			   OUT ULONG *Timeout, OUT USHORT *ListenIntv);

BOOLEAN MlmeAuthReqSanity(IN RTMP_ADAPTER *pAd, IN VOID *Msg, IN ULONG MsgLen,
			  OUT PUCHAR pAddr, OUT ULONG *Timeout,
			  OUT USHORT *Alg);

BOOLEAN MlmeStartReqSanity(IN RTMP_ADAPTER *pAd, IN VOID *Msg, IN ULONG MsgLen,
			   OUT CHAR Ssid[], OUT UCHAR *Ssidlen);

BOOLEAN PeerAuthSanity(IN RTMP_ADAPTER *pAd, IN VOID *Msg, IN ULONG MsgLen,
		       OUT PUCHAR pAddr, OUT USHORT *Alg, OUT USHORT *Seq,
		       OUT USHORT *Status, OUT CHAR ChlgText[]);

BOOLEAN PeerAssocRspSanity(
	IN struct wifi_dev *wdev, IN VOID *pMsg, IN ULONG MsgLen,
	OUT PUCHAR pAddr2, OUT USHORT *pCapabilityInfo, OUT USHORT *pStatus,
	OUT USHORT *pAid, OUT UCHAR SupRate[], OUT UCHAR *pSupRateLen,
	OUT UCHAR ExtRate[], OUT UCHAR *pExtRateLen,
	OUT HT_CAPABILITY_IE *pHtCapability,
	OUT ADD_HT_INFO_IE
		*pAddHtInfo, /* AP might use this additional ht info IE */
	OUT UCHAR *pHtCapabilityLen, OUT UCHAR *pAddHtInfoLen,
	OUT UCHAR *pNewExtChannelOffset, OUT PEDCA_PARM pEdcaParm,
	OUT EXT_CAP_INFO_ELEMENT *pExtCapInfo, OUT UCHAR *pCkipFlag,
	struct _IE_lists *ie_list);

BOOLEAN PeerDisassocSanity(IN RTMP_ADAPTER *pAd, IN VOID *Msg, IN ULONG MsgLen,
			   OUT PUCHAR pAddr2, OUT USHORT *Reason);

BOOLEAN PeerDeauthSanity(IN RTMP_ADAPTER *pAd, IN VOID *Msg, IN ULONG MsgLen,
			 OUT PUCHAR pAddr1, OUT PUCHAR pAddr2,
			 OUT PUCHAR pAddr3, OUT USHORT *Reason);

BOOLEAN GetTimBit(IN CHAR *Ptr, IN USHORT Aid, OUT UCHAR *TimLen,
...
UCHAR ChannelSanityByWdev(IN PRTMP_ADAPTER pAd, IN struct wifi_dev *wdev,
			  IN UCHAR channel);
...
BOOLEAN MlmeAddBAReqSanity(IN RTMP_ADAPTER *pAd, IN VOID *Msg, IN ULONG MsgLen,
			   OUT PUCHAR pAddr2);
...
VOID MlmePeriodicExec(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
		      IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

VOID MlmePeriodicExecTimer(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			   IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

VOID LinkDownExec(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
		  IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

VOID LinkUpExec(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
		IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

VOID STAMlmePeriodicExec(RTMP_ADAPTER *pAd, struct wifi_dev *wdev);

VOID MlmeAutoScan(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev);

VOID MlmeAutoReconnectLastSSID(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev);

BOOLEAN MlmeValidateSSID(IN PUCHAR pSsid, IN UCHAR SsidLen);

VOID MlmeCheckForRoaming(IN RTMP_ADAPTER *pAd, IN ULONG Now32);

BOOLEAN MlmeCheckForFastRoaming(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev);

#ifdef TXBF_SUPPORT
BOOLEAN MlmeTxBfAllowed(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry,
...
VOID MlmeSetPsmBit(IN RTMP_ADAPTER *pAd, IN PSTA_ADMIN_CONFIG pStaCfg,
		   IN USHORT psm);
...
VOID UpdateBasicRateBitmap(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev);

VOID MlmeUpdateTxRates(IN RTMP_ADAPTER *pAd, IN BOOLEAN bLinkUp,
		       IN UCHAR apidx);

VOID MlmeUpdateTxRatesWdev(RTMP_ADAPTER *pAd, BOOLEAN bLinkUp,
			   struct wifi_dev *wdev);
...
#ifdef DOT11_VHT_AC
VOID MlmeUpdateVhtTxRates(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry,
			  IN UCHAR apidx);
#endif /* DOT11_VHT_AC */

#endif /* DOT11_N_SUPPORT */

VOID RTMPCheckRates(IN RTMP_ADAPTER *pAd, IN OUT UCHAR SupRate[],
		    IN OUT UCHAR *SupRateLen, IN UCHAR PhyMode);

BOOLEAN RTMPCheckHt(IN RTMP_ADAPTER *pAd, IN UCHAR Wcid,
		    INOUT HT_CAPABILITY_IE *pHtCapability,
		    INOUT ADD_HT_INFO_IE *pAddHtInfo);

#ifdef DOT11_VHT_AC
BOOLEAN RTMPCheckVht(IN RTMP_ADAPTER *pAd, IN UCHAR Wcid,
		     IN VHT_CAP_IE *vht_cap, IN VHT_OP_IE *vht_op);
#endif /* DOT11_VHT_AC */

VOID RTMPUpdateMlmeRate(IN RTMP_ADAPTER *pAd);

CHAR RTMPMaxRssi(IN RTMP_ADAPTER *pAd, IN CHAR Rssi0, IN CHAR Rssi1,
		 IN CHAR Rssi2
#if defined(CUSTOMER_DCC_FEATURE) || defined(CONFIG_MAP_SUPPORT)
		 ,
		 IN CHAR Rssi3
...
CHAR RTMPMinRssi(IN RTMP_ADAPTER *pAd, IN CHAR Rssi0, IN CHAR Rssi1,
		 IN CHAR Rssi2, IN CHAR Rssi3);
...
#ifdef MICROWAVE_OVEN_SUPPORT
INT Set_MO_FalseCCATh_Proc(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);
...
UCHAR eFuseReadRegisters(RTMP_ADAPTER *pAd, USHORT Offset, UINT16 Length,
			 UINT16 *pData);
VOID EfusePhysicalReadRegisters(RTMP_ADAPTER *pAd, USHORT Offset, USHORT Length,
				USHORT *pData);
...
VOID AsicRxAntEvalTimeout(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			  IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

VOID APSDPeriodicExec(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
		      IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

BOOLEAN RTMPCheckEntryEnableAutoRateSwitch(IN RTMP_ADAPTER *pAd,
					   IN PMAC_TABLE_ENTRY pEntry);

UCHAR RTMPStaFixedTxMode(IN RTMP_ADAPTER *pAd, IN PMAC_TABLE_ENTRY pEntry);

VOID RTMPUpdateLegacyTxSetting(IN UCHAR fixed_tx_mode,
			       IN PMAC_TABLE_ENTRY pEntry);
...
#ifdef DFS_VENDOR10_CUSTOM_FEATURE
BOOLEAN ApAutoChannelSkipListBuild(IN RTMP_ADAPTER *pAd,
				   IN struct wifi_dev *wdev);
#endif
...
VOID RTMPInitMICEngine(IN RTMP_ADAPTER *pAd, IN PUCHAR pKey, IN PUCHAR pDA,
		       IN PUCHAR pSA, IN UCHAR UserPriority, IN PUCHAR pMICKey);

BOOLEAN RTMPTkipCompareMICValue(IN RTMP_ADAPTER *pAd, IN PUCHAR pSrc,
				IN PUCHAR pDA, IN PUCHAR pSA, IN PUCHAR pMICKey,
				IN UCHAR UserPriority, IN UINT Len);

VOID RTMPCalculateMICValue(IN RTMP_ADAPTER *pAd, IN PNDIS_PACKET pPacket,
			   IN PUCHAR pEncap, IN PUCHAR pKey, IN PUCHAR pMIC,
			   IN UCHAR apidx);
...
INT RT_CfgSetCountryRegion(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg,
			   IN INT band);
...
INT RT_CfgSetWepKey(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *keyString,
...
#ifdef WSC_INCLUDED
INT RT_CfgSetWscPinCode(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *pPinCodeStr,
...
INT Set_RadioOn_Proc(IN PRTMP_ADAPTER pAd, IN RTMP_STRING *arg);

#ifdef CUSTOMER_RSG_FEATURE
INT Set_ApEnableRadioChStats_Proc(IN PRTMP_ADAPTER pAd, IN RTMP_STRING *arg);
#endif
#ifdef CUSTOMER_DCC_FEATURE
INT Set_ApDisableSTAConnect_Proc(IN PRTMP_ADAPTER pAd, IN RTMP_STRING *arg);

INT Set_ApEnableBeaconTable_Proc(IN PRTMP_ADAPTER pAd, IN RTMP_STRING *arg);

VOID RTMPIoctlQuerySTAStat(IN PRTMP_ADAPTER pAd,
			   IN RTMP_IOCTL_INPUT_STRUCT *wrq);

VOID RTMPIoctlQueryMbssStat(IN PRTMP_ADAPTER pAd,
			    IN RTMP_IOCTL_INPUT_STRUCT *wrq);
#endif

#ifdef NEW_SET_RX_STREAM
INT Set_RxStream_Proc(IN PRTMP_ADAPTER pAd, IN RTMP_STRING *arg);
#endif

INT Set_Lp_Proc(IN PRTMP_ADAPTER pAd, IN RTMP_STRING *arg);

INT set_fix_amsdu(PRTMP_ADAPTER pAd, char *arg);

INT set_rx_max_cnt(PRTMP_ADAPTER pAd, char *arg);

INT set_rx1_max_cnt(PRTMP_ADAPTER pAd, char *arg);

INT set_rx_dly_ctl(PRTMP_ADAPTER pAd, char *arg);

INT set_tx_dly_ctl(PRTMP_ADAPTER pAd, char *arg);

#ifdef MAP_R2
INT show_traffic_separation_info(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);
#endif

#ifdef PCIE_ASPM_DYM_CTRL_SUPPORT
void set_pcie_aspm_dym_ctrl_cap(PRTMP_ADAPTER pAd,
				BOOLEAN flag_pcie_aspm_dym_ctrl);

BOOLEAN get_pcie_aspm_dym_ctrl_cap(PRTMP_ADAPTER pAd);
...
VOID RTMPWPARemoveAllKeys(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev);

BOOLEAN RTMPCheckStrPrintAble(IN CHAR *pInPutStr, IN UCHAR strLen);

VOID RTMPSetDefaultChannel(IN PRTMP_ADAPTER pAd);

VOID RTMPUpdateRateInfo(UCHAR phymode, struct dev_rate_info *rate);
VOID RTMPSetPhyMode(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev,
		    IN UCHAR phymode);
...
VOID RTMPAddWcidAttributeEntry(IN RTMP_ADAPTER *pAd, IN UCHAR BssIdx,
			       IN UCHAR KeyIdx, IN UCHAR CipherAlg,
...
#ifdef DOT11_N_SUPPORT
VOID set_sta_ht_cap(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *ent,
...
#define MAKE_IE_TO_BUF(__BUF, __CONTENT, __CONTENT_LEN, __CUR_LEN)             \
	{                                                                      \
		NdisMoveMemory((__BUF + __CUR_LEN), (UCHAR *)(__CONTENT),      \
			       __CONTENT_LEN);                                 \
...
#ifdef SYSTEM_LOG_SUPPORT
VOID RtmpDrvSendWirelessEvent(IN VOID *pAdSrc, IN USHORT Event_flag,
			      IN PUCHAR pAddr, IN UCHAR wdev_idx, IN CHAR Rssi);
#else
#define RtmpDrvSendWirelessEvent(_pAd, _Event_flag, _pAddr, wdev_idx, _Rssi)
#endif /* SYSTEM_LOG_SUPPORT */

CHAR ConvertToRssi(IN RTMP_ADAPTER *pAd, IN struct raw_rssi_info *rssi_info,
		   IN UCHAR rssi_idx);

CHAR ConvertToSnr(RTMP_ADAPTER *pAd, UCHAR Snr);

BOOLEAN AdjustBwToSyncAp(RTMP_ADAPTER *pAd, BCN_IE_LIST *ie_list, UCHAR Wcid,
			 UCHAR fun_tb_idx);

#ifdef DOT11N_DRAFT3
VOID BuildEffectedChannelList(IN RTMP_ADAPTER *pAd, IN struct wifi_dev *wdev);

VOID CntlChannelWidth(IN RTMP_ADAPTER *pAd, IN UCHAR PrimaryChannel,
		      IN UCHAR CentralChannel, IN UCHAR ChannelWidth,
...
VOID WPAStart4WayHS(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY *pEntry,
...
#ifdef TXBF_SUPPORT
#ifndef MT_MAC
VOID eTxBfProbeTimerExec(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			 IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);
#endif /* MT_MAC */
#endif /* TXBF_SUPPORT */

VOID RTMPHandleSTAKey(IN RTMP_ADAPTER *pAd, IN MAC_TABLE_ENTRY *pEntry,
		      IN MLME_QUEUE_ELEM *Elem);

VOID MlmeDeAuthAction(IN RTMP_ADAPTER *pAd, IN PMAC_TABLE_ENTRY pEntry,
		      IN USHORT Reason, IN BOOLEAN bDataFrameFirst);

#ifdef DOT11W_PMF_SUPPORT
VOID PMF_SAQueryTimeOut(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);
...
BOOLEAN fill_tx_blk(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
		    struct _TX_BLK *tx_blk);
UINT16 tx_pkt_classification(RTMP_ADAPTER *pAd, PNDIS_PACKET pPacket,
			     TX_BLK *pTxBlk);
INT send_data_pkt(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, PNDIS_PACKET pkt);
INT send_mlme_pkt(RTMP_ADAPTER *pAd, PNDIS_PACKET pkt, struct wifi_dev *wdev,
		  UCHAR q_idx, BOOLEAN is_data_queue);
INT32 fp_send_data_pkt(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
		       PNDIS_PACKET pkt);

void announce_802_3_packet(IN VOID *pAdSrc, IN PNDIS_PACKET pPacket,
			   IN UCHAR OpMode);

UINT announce_amsdu_pkt(RTMP_ADAPTER *pAd, PNDIS_PACKET pPacket, UCHAR OpMode);
...
VOID ba_ori_session_setup(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, UCHAR TID,
			  USHORT TimeOut, ULONG DelayTime, BOOLEAN isForced);

BOOLEAN ba_rec_session_add(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry,
			   FRAME_ADDBA_REQ *pFrame);

VOID ba_session_tear_down_all(RTMP_ADAPTER *pAd, UCHAR Wcid);

VOID ba_ori_session_tear_down(RTMP_ADAPTER *pAd, UCHAR Wcid, UCHAR TID,
...
VOID ba_rec_session_tear_down(RTMP_ADAPTER *pAd, UCHAR Wcid, UCHAR TID,
			      BOOLEAN bPassive);
#endif /* DOT11_N_SUPPORT */
...
UINT GenerateWpsPinCode(IN RTMP_ADAPTER *pAd, IN BOOLEAN bFromApcli,
			IN UCHAR apidx);
...
VOID WscStateMachineInit(IN RTMP_ADAPTER *pAd, IN STATE_MACHINE *S,
			 OUT STATE_MACHINE_FUNC Trans[]);
...
VOID WscEapEnrolleeAction(IN RTMP_ADAPTER *pAd, IN MLME_QUEUE_ELEM *Elem,
			  IN UCHAR MsgType, IN MAC_TABLE_ENTRY *pEntry,
			  IN PWSC_CTRL pWscControl);

#ifdef CONFIG_AP_SUPPORT
VOID WscEapApProxyAction(IN RTMP_ADAPTER *pAd, IN MLME_QUEUE_ELEM *Elem,
			 IN UCHAR MsgType, IN MAC_TABLE_ENTRY *pEntry,
			 IN PWSC_CTRL pWscControl);
#endif /* CONFIG_AP_SUPPORT */

VOID WscEapRegistrarAction(IN RTMP_ADAPTER *pAd, IN MLME_QUEUE_ELEM *Elem,
			   IN UCHAR MsgType, IN MAC_TABLE_ENTRY *pEntry,
			   IN PWSC_CTRL pWscControl);

VOID WscEAPOLTimeOutAction(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			   IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);
...
VOID WscInitRegistrarPair(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
			  IN UCHAR apidx);

VOID WscSendEapReqId(IN RTMP_ADAPTER *pAd, IN PMAC_TABLE_ENTRY pEntry,
		     IN UCHAR CurOpMode);

VOID WscSendEapolStart(IN RTMP_ADAPTER *pAd, IN PUCHAR pBssid,
		       IN UCHAR CurOpMode, IN VOID *wdev_obj);

VOID WscSendEapRspId(IN RTMP_ADAPTER *pAd, IN PMAC_TABLE_ENTRY pEntry,
		     IN PWSC_CTRL pWscControl);

VOID WscMacHeaderInit(IN RTMP_ADAPTER *pAd, IN OUT PHEADER_802_11 Hdr,
		      IN PUCHAR pAddr1, IN PUCHAR pBSSID,
		      IN BOOLEAN bFromApCli);

VOID WscSendMessage(
	IN RTMP_ADAPTER *pAd, IN UCHAR OpCode, IN PUCHAR pData, IN INT Len,
	IN PWSC_CTRL pWscControl,
...
VOID WscSendEapFail(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		    IN BOOLEAN bSendDeAuth);

VOID WscM2DTimeOutAction(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			 IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

VOID WscUPnPMsgTimeOutAction(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			     IN PVOID SystemSpecific2,
			     IN PVOID SystemSpecific3);

int WscSendUPnPConfReqMsg(IN RTMP_ADAPTER *pAd, IN UCHAR apIdx,
			  IN PUCHAR ssidStr, IN PUCHAR macAddr, IN INT Status,
			  IN UINT eventID, IN UCHAR CurOpMode);

int WscSendUPnPMessage(IN RTMP_ADAPTER *pAd, IN UCHAR apIdx, IN USHORT msgType,
		       IN USHORT msgSubType, IN PUCHAR pData, IN INT dataLen,
		       IN UINT eventID, IN UINT toIPAddr, IN PUCHAR pMACAddr,
		       IN UCHAR CurOpMode);

VOID WscUPnPErrHandle(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		      IN UINT eventID);

VOID WscBuildBeaconIE(IN RTMP_ADAPTER *pAd, IN UCHAR b_configured,
		      IN BOOLEAN b_selRegistrar, IN USHORT devPwdId,
		      IN USHORT selRegCfgMethods, IN UCHAR apidx,
		      IN UCHAR *pAuthorizedMACs, IN UCHAR AuthorizedMACsLen,
		      IN UCHAR CurOpMode);

VOID WscBuildProbeRespIE(IN RTMP_ADAPTER *pAd, IN UCHAR respType,
			 IN UCHAR scState, IN BOOLEAN b_selRegistrar,
			 IN USHORT devPwdId, IN USHORT selRegCfgMethods,
			 IN UCHAR apidx, IN UCHAR *pAuthorizedMACs,
			 IN INT AuthorizedMACsLen, IN UCHAR CurOpMode);

#ifdef CONFIG_AP_SUPPORT
VOID WscBuildAssocRespIE(IN RTMP_ADAPTER *pAd, IN UCHAR ApIdx, IN UCHAR Reason,
			 OUT PUCHAR pOutBuf, OUT PUCHAR pIeLen);

VOID WscSelectedRegistrar(IN RTMP_ADAPTER *pAd, IN PUCHAR RegInfo,
			  IN UINT length, IN UCHAR apidx);

VOID WscInformFromWPA(IN PMAC_TABLE_ENTRY pEntry);

#ifdef WSC_AP_SUPPORT
#ifdef APCLI_SUPPORT
VOID WscApCliLinkDownById(IN PRTMP_ADAPTER pAd, IN UCHAR apidx);
#endif /* APCLI_SUPPORT */
#endif /* WSC_AP_SUPPORT */

#endif /* CONFIG_AP_SUPPORT */

VOID WscBuildProbeReqIE(IN RTMP_ADAPTER *pAd, IN VOID *wdev_obj,
			OUT PUCHAR pOutBuf, OUT PUCHAR pIeLen);

VOID WscBuildAssocReqIE(IN PWSC_CTRL pWscControl, OUT PUCHAR pOutBuf,
			OUT PUCHAR pIeLen);

VOID WscProfileRetryTimeout(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			    IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

VOID WscPBCTimeOutAction(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			 IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);
#ifdef CON_WPS
VOID WscScanDoneCheckTimeOutAction(IN PVOID SystemSpecific1,
				   IN PVOID FunctionContext,
				   IN PVOID SystemSpecific2,
				   IN PVOID SystemSpecific3);
#endif /*CON_WPS*/
...
VOID WscPINTimeOutAction(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			 IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);
#endif

VOID WscScanTimeOutAction(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			  IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

INT WscGenerateUUID(RTMP_ADAPTER *pAd, UCHAR *uuidHexStr, UCHAR *uuidAscStr,
		    int apIdx, BOOLEAN bUseCurrentTime, BOOLEAN from_apcli);
...
#ifdef CON_WPS
VOID WscConWpsStop(IN PRTMP_ADAPTER pAd, IN BOOLEAN bFromApCli,
		   IN PWSC_CTRL pWscControl);
#endif /* CON_WPS */
...
int BuildMessageM1(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf);

int BuildMessageM2(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf);

int BuildMessageM2D(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		    OUT VOID *pbuf);

int BuildMessageM3(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf);

int BuildMessageM4(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf);

int BuildMessageM5(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf);

int BuildMessageM6(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf);

int BuildMessageM7(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf);

int BuildMessageM8(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		   OUT VOID *pbuf);

int BuildMessageDONE(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		     OUT VOID *pbuf);

int BuildMessageACK(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		    OUT VOID *pbuf);

int BuildMessageNACK(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		     OUT VOID *pbuf);

int ProcessMessageM1(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		     IN VOID *precv, IN INT Length, OUT PWSC_REG_DATA pReg);

int ProcessMessageM2(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		     IN VOID *precv, IN INT Length, IN UCHAR apidx,
		     OUT PWSC_REG_DATA pReg);

int ProcessMessageM2D(IN RTMP_ADAPTER *pAd, IN VOID *precv, IN INT Length,
		      OUT PWSC_REG_DATA pReg);

int ProcessMessageM3(IN RTMP_ADAPTER *pAd, IN VOID *precv, IN INT Length,
		     OUT PWSC_REG_DATA pReg);

int ProcessMessageM4(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		     IN VOID *precv, IN INT Length, OUT PWSC_REG_DATA pReg);

int ProcessMessageM5(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		     IN VOID *precv, IN INT Length, OUT PWSC_REG_DATA pReg);

int ProcessMessageM6(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		     IN VOID *precv, IN INT Length, OUT PWSC_REG_DATA pReg);

int ProcessMessageM7(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		     IN VOID *precv, IN INT Length, OUT PWSC_REG_DATA pReg);

int ProcessMessageM8(IN RTMP_ADAPTER *pAd, IN VOID *precv, IN INT Length,
		     IN PWSC_CTRL pWscControl);

USHORT WscGetAuthType(IN UINT32 authType);

USHORT WscGetEncryType(IN UINT32 encryType);

NDIS_STATUS WscThreadInit(RTMP_ADAPTER *pAd);

BOOLEAN WscThreadExit(RTMP_ADAPTER *pAd);

int AppendWSCTLV(IN USHORT index, OUT UCHAR *obuf, IN UCHAR *ibuf,
		 IN USHORT varlen);

VOID WscGetRegDataPIN(IN RTMP_ADAPTER *pAd, IN UINT PinCode,
		      IN PWSC_CTRL pWscControl);

VOID WscPushPBCAction(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl);

#ifdef WSC_STA_SUPPORT
VOID WscPINAction(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl);

BOOLEAN WscPINExec(IN RTMP_ADAPTER *pAd, IN BOOLEAN bFromM2,
		   IN PWSC_CTRL pWscControl);

VOID WscPINBssTableSort(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl);
#endif

VOID WscScanExec(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl);

BOOLEAN WscPBCExec(IN RTMP_ADAPTER *pAd, IN BOOLEAN bFromM2,
		   IN PWSC_CTRL pWscControl);

VOID WscPBCBssTableSort(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl);

VOID WscGenRandomKey(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		     INOUT PUCHAR pKey, INOUT PUSHORT pKeyLen);
...
void WscWriteConfToPortCfg(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
			   IN PWSC_CREDENTIAL pCredential,
			   IN BOOLEAN bEnrollee);

#ifdef APCLI_SUPPORT
void WscWriteConfToApCliCfg(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
			    IN PWSC_CREDENTIAL pCredential,
			    IN BOOLEAN bEnrollee);
#endif /* APCLI_SUPPORT */
...
VOID WscPBCSessionOverlapCheck(IN RTMP_ADAPTER *pAd, IN UCHAR current_band);

VOID WscPBC_DPID_FromSTA(IN RTMP_ADAPTER *pAd, IN PUCHAR pMacAddr,
			 IN UCHAR current_band);

#ifdef CONFIG_AP_SUPPORT
INT WscGetConfWithoutTrigger(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
			     IN BOOLEAN bFromUPnP);

BOOLEAN WscReadProfileFromUfdFile(IN RTMP_ADAPTER *pAd, IN UCHAR ApIdx,
				  IN RTMP_STRING *pUfdFileName);

BOOLEAN WscWriteProfileToUfdFile(IN RTMP_ADAPTER *pAd, IN UCHAR ApIdx,
				 IN RTMP_STRING *pUfdFileName);
#endif /* CONFIG_AP_SUPPORT */
...
VOID WscLEDTimer(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
		 IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

VOID WscSkipTurnOffLEDTimer(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			    IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);
#endif /* WSC_LED_SUPPORT */

#ifdef CONFIG_AP_SUPPORT
VOID WscUpdatePortCfgTimeout(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			     IN PVOID SystemSpecific2,
			     IN PVOID SystemSpecific3);
#endif /* CONFIG_AP_SUPPORT */

VOID WscCheckPeerDPID(IN RTMP_ADAPTER *pAd, IN PFRAME_802_11 Fr,
		      IN PUCHAR eid_data, IN INT eid_len,
		      IN UCHAR current_band);
...
VOID WscDeleteEntryFromAclList(RTMP_ADAPTER *pAd, INT ApIdx, UCHAR *pMacAddr);
...
BOOLEAN WscGenV2Msg(IN PWSC_CTRL pWpsCtrl, IN BOOLEAN bSelRegistrar,
		    IN PUCHAR pAuthorizedMACs, IN INT AuthorizedMACsLen,
		    OUT UCHAR **pOutBuf, OUT INT *pOutBufLen);

BOOLEAN WscParseV2SubItem(IN UCHAR SubID, IN PUCHAR pData, IN USHORT DataLen,
			  OUT PUCHAR pOutBuf, OUT PUCHAR pOutBufLen);

VOID WscSendEapFragAck(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
		       IN PMAC_TABLE_ENTRY pEntry);

VOID WscSendEapFragData(IN RTMP_ADAPTER *pAd, IN PWSC_CTRL pWscControl,
			IN PMAC_TABLE_ENTRY pEntry);
#endif /* WSC_V2_SUPPORT */

BOOLEAN WscGetDataFromPeerByTag(IN RTMP_ADAPTER *pAd, IN PUCHAR pIeData,
				IN INT IeDataLen, IN USHORT WscTag,
				OUT PUCHAR pWscBuf, OUT PUSHORT pWscBufLen);

VOID WscUUIDInit(IN PRTMP_ADAPTER pAd, IN INT inf_idx, IN UCHAR from_apcli);
...
#ifdef CUSTOMER_DCC_FEATURE
INT Set_ApChannelSwitch_Proc(IN PRTMP_ADAPTER pAd, IN RTMP_STRING *arg);
#endif
...
#ifdef THERMAL_PROTECT_SUPPORT
INT set_thermal_protection_criteria_proc(IN RTMP_ADAPTER *pAd,
					 IN RTMP_STRING *arg);

INT set_thermal_protection_admin_ctrl_duty_proc(IN RTMP_ADAPTER *pAd,
						IN RTMP_STRING *arg);
#endif /* THERMAL_PROTECT_SUPPORT */
...
#define IS_V10_OLD_CHNL_VALID(_wdev) (_wdev->bV10OldChannelValid == TRUE)
...
#define IS_APCLI_BW_SYNC_FEATURE_ENBL(_pAd)                                    \
	((BOOLEAN)(_pAd->ApCfg.ApCliAutoBWRules.minorPolicy                    \
			   .ApCliBWSyncHTSupport) ||                           \
	 (_pAd->ApCfg.ApCliAutoBWRules.minorPolicy.ApCliBWSyncVHTSupport))

#define IS_V10_AUTO_BAND_FEATURE_ENBL(_pAd)                                    \
	((BOOLEAN)(_pAd->ApCfg.ApCliAutoBWRules.majorPolicy                    \
			   .ApCliBWSyncBandSupport) ||                         \
...
#define SET_APCLI_AUTO_BW_HT_VALID(_pAd, valid)                                \
	(_pAd->ApCfg.ApCliAutoBWRules.minorPolicy.ApCliBWSyncHTSupport |=      \
	 (1 << valid))
...
#define IS_APCLI_SYNC_BAND_VALID(_pAd, condition)                              \
	(1 &                                                                   \
	 (_pAd->ApCfg.ApCliAutoBWRules.majorPolicy.ApCliBWSyncBandSupport >>   \
	  condition))
...
#define IS_APCLI_SYNC_PEER_DEAUTH_ENBL(_pAd) (_pAd->ApCfg.AutoBWDeauthEnbl)
...
RTMP_STRING *GetAuthModeStr(IN UINT32 authMode);

RTMP_STRING *GetEncryModeStr(IN UINT32 encryMode);

UINT32 SecAuthModeOldToNew(IN USHORT authMode);

UINT32 SecEncryModeOldToNew(IN USHORT encryMode);

USHORT SecAuthModeNewToOld(IN UINT32 authMode);

USHORT SecEncryModeNewToOld(IN UINT32 encryMode);

VOID detect_wmm_traffic(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UCHAR up,
			UCHAR bOutput);
#ifdef CONFIG_AP_SUPPORT
...
void build_ext_channel_switch_ie(IN RTMP_ADAPTER *pAd,
				 ...
				 IN UCHAR Channel, IN UCHAR PhyMode,
				 IN struct wifi_dev *wdev);
#endif /*defined (A_BAND_SUPPORT) && defined (CONFIG_AP_SUPPORT)*/

void assoc_ht_info_debugshow(IN RTMP_ADAPTER *pAd, IN MAC_TABLE_ENTRY *pEntry,
			     IN UCHAR ht_cap_len,
			     IN HT_CAPABILITY_IE *pHTCapability);
#endif /* DOT11_N_SUPPORT */

INT header_packet_process(RTMP_ADAPTER *pAd, PNDIS_PACKET pRxPacket,
			  RX_BLK *pRxBlk);

NDIS_STATUS rx_packet_process(RTMP_ADAPTER *pAd, PNDIS_PACKET pRxPacket,
			      RX_BLK *pRxBlk);
...
UINT deaggregate_amsdu_announce(IN RTMP_ADAPTER *pAd, PNDIS_PACKET pPacket,
...
VOID indicate_agg_ralink_pkt(IN RTMP_ADAPTER *pAd, IN MAC_TABLE_ENTRY *pEntry,
			     IN RX_BLK *pRxBlk, IN UCHAR wdev_idx);

#ifdef CUSTOMER_DCC_FEATURE
VOID Update_Snr_Sample(IN RTMP_ADAPTER *pAd, IN MAC_TABLE_ENTRY *pEntry,
		       IN struct rx_signal_info *signal);
#endif

VOID Update_Rssi_Sample(IN RTMP_ADAPTER *pAd, IN RSSI_SAMPLE *pRssi,
			IN struct rx_signal_info *signal, IN UCHAR phymode,
			IN UCHAR bw);

UINT32 mtd_pci_get_resource_idx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev,
				enum PACKET_TYPE, UCHAR q_idx);

PNDIS_PACKET mtd_pci_get_pkt_from_rx_resource(RTMP_ADAPTER *pAd,
					      BOOLEAN *pbReschedule,
					      UINT32 *pRxPending,
					      UCHAR RxRingNo);

PNDIS_PACKET mtd_pci_get_pkt_from_rx_resource_io(RTMP_ADAPTER *pAd,
						 BOOLEAN *pbReschedule,
						 UINT32 *pRxPending,
						 UCHAR RxRingNo);

PNDIS_PACKET pci_get_pkt_from_rx_resource(RTMP_ADAPTER *pAd,
					  BOOLEAN *pbReschedule,
					  UINT32 *pRxPending, UCHAR RxRingNo);

PNDIS_PACKET usb_get_pkt_from_rx_resource(RTMP_ADAPTER *pAd,
					  BOOLEAN *pbReschedule,
					  UINT32 *pRxPending, UCHAR RxRingNo);

PNDIS_PACKET sdio_get_pkt_from_rx_resource(RTMP_ADAPTER *pAd,
					   BOOLEAN *pbReschedule,
					   UINT32 *pRxPending, UCHAR RxRingNo);

UCHAR *mt_pci_get_hif_buf(RTMP_ADAPTER *pAd, struct _TX_BLK *tx_blk,
			  UCHAR hif_idx, UCHAR frame_type);

VOID de_fragment_data_pkt(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk);

#if defined(AP_SCAN_SUPPORT) || defined(CONFIG_STA_SUPPORT)
VOID RTMPIoctlGetSiteSurvey(IN RTMP_ADAPTER *pAd,
			    IN RTMP_IOCTL_INPUT_STRUCT *wrq);
#endif

#ifdef CUSTOMER_RSG_FEATURE
VOID RTMPIoctlGetRadioStatsCount(IN PRTMP_ADAPTER pAdapter,
				 IN RTMP_IOCTL_INPUT_STRUCT *wrq);
#endif

#ifdef CONFIG_AP_SUPPORT
#ifdef CUSTOMER_DCC_FEATURE
UINT32 GetNumberofSpatialStreams(IN HT_CAPABILITY_IE htCapabilityIE);

VOID RTMPIoctlGetStreamType(IN PRTMP_ADAPTER pAd,
			    IN RTMP_IOCTL_INPUT_STRUCT *wrq);

VOID RTMPIoctlGetApTable(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq);

VOID RTMPIoctlGetApScanResults(IN PRTMP_ADAPTER pAdapter,
			       IN RTMP_IOCTL_INPUT_STRUCT *wrq);
...
BOOLEAN ApCliDoRoamingRefresh(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry,
...
PUCHAR MATEngineTxHandle(RTMP_ADAPTER *pAd, PNDIS_PACKET pPkt, UINT infIdx,
			 UCHAR OpMode);
...
VOID MtUpdateBeaconToAsic(IN RTMP_ADAPTER *pAd, VOID *wdev_void,
			  IN UINT16 FrameLen, IN UCHAR UpdatePktType);
...
#ifdef RTMP_MAC_PCI
BOOLEAN RT28xxPciAsicRadioOff(IN RTMP_ADAPTER *pAd, IN UCHAR Level,
...
ra_dma_addr_t RtmpDrvPciMapSingle(IN RTMP_ADAPTER *pAd, IN VOID *ptr,
				  IN size_t size, IN INT sd_idx,
				  IN INT direction);
...
/*///////////////////////////////////*/
INT RTMPShowCfgValue(RTMP_ADAPTER *pAd, RTMP_STRING *name, RTMP_STRING *buf,
		     UINT32 MaxLen);
/*//////////////////////////////////*/
...
VOID RTMPUpdateSwCacheCipherInfo(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk,
				 UCHAR *pHdr);
INT tx_sw_encrypt(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk, UCHAR *buf_ptr,
...
NDIS_STATUS RTEnqueueInternalCmd(IN RTMP_ADAPTER *pAd, IN NDIS_OID Oid,
...
VOID ApCliRTMPSendNullFrame(IN RTMP_ADAPTER *pAd, IN UCHAR TxRate,
			    IN BOOLEAN bQosNull, IN MAC_TABLE_ENTRY *pMacEntry,
			    IN USHORT PwrMgmt);

#endif /*APCLI_SUPPORT*/

void RTMP_IndicateMediaState(IN RTMP_ADAPTER *pAd,
			     IN NDIS_MEDIA_STATE media_state);

INT RTMPSetInformation(IN RTMP_ADAPTER *pAd, IN OUT RTMP_IOCTL_INPUT_STRUCT *rq,
		       IN INT cmd, IN struct wifi_dev *wdev);

INT RTMPQueryInformation(IN RTMP_ADAPTER *pAd,
			 INOUT RTMP_IOCTL_INPUT_STRUCT *rq, IN INT cmd,
			 IN struct wifi_dev *wdev);

VOID RTMPIoctlShow(IN RTMP_ADAPTER *pAd, IN RTMP_IOCTL_INPUT_STRUCT *rq,
...
NDIS_STATUS RtmpEnqueueTokenFrame(IN RTMP_ADAPTER *pAd, IN UCHAR *pAddr,
				  IN UCHAR TxRate, IN UCHAR AID, IN UCHAR apidx,
				  IN UCHAR OldUP);

VOID RtmpEnqueueNullFrame(IN RTMP_ADAPTER *pAd, IN UCHAR *pAddr,
			  IN UCHAR TxRate, IN UCHAR AID, IN UCHAR apidx,
			  IN BOOLEAN bQosNull, IN BOOLEAN bEOSP,
			  IN UCHAR OldUP);

VOID RtmpCleanupPsQueue(IN RTMP_ADAPTER *pAd, IN PQUEUE_HEADER pQueue);

NDIS_STATUS RtmpInsertPsQueue(IN RTMP_ADAPTER *pAd, IN PNDIS_PACKET pPacket,
			      IN MAC_TABLE_ENTRY *pMacEntry, IN UCHAR QueIdx);

VOID RtmpHandleRxPsPoll(RTMP_ADAPTER *pAd, UCHAR *pAddr, USHORT Aid,
			BOOLEAN isActive);
...
VOID MtEnqTxSwqFromPsQueue(RTMP_ADAPTER *pAd, UCHAR qidx,
			   STA_TR_ENTRY *tr_entry);
VOID CheckSkipTX(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry);
#endif /* MT_MAC */

#ifdef CONFIG_MULTI_CHANNEL
VOID RtmpEnqueueLastNullFrame(IN RTMP_ADAPTER *pAd, IN PUCHAR pAddr,
			      IN UCHAR TxRate, IN UCHAR PID, IN UCHAR apidx,
			      IN BOOLEAN bQosNull, IN BOOLEAN bEOSP,
			      IN UCHAR OldUP, IN UCHAR PwrMgmt,
			      IN UCHAR OpMode);

VOID EnableMACTxPacket(IN RTMP_ADAPTER *pAd, IN PMAC_TABLE_ENTRY pEntry,
		       IN UCHAR PwrMgmt, IN BOOLEAN bTxNullFramei,
		       IN UCHAR QSel);

VOID DisableMACTxPacket(IN RTMP_ADAPTER *pAd, IN PMAC_TABLE_ENTRY pEntry,
			IN UCHAR PwrMgmt, IN BOOLEAN bWaitACK, IN UCHAR QSel);

VOID InitMultiChannelRelatedValue(IN RTMP_ADAPTER *pAd, IN UCHAR Channel,
				  IN UCHAR CentralChannel);

VOID EDCA_ActionTimeout(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

VOID HCCA_ActionTimeout(IN PVOID SystemSpecific1, IN PVOID FunctionContext,
			IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);
...
UCHAR dot11_max_sup_rate(INT SupRateLen, UCHAR *SupRate, INT ExtRateLen,
			 UCHAR *ExtRate);

VOID TRTableResetEntry(RTMP_ADAPTER *pAd, UCHAR tr_tb_idx);
VOID TRTableInsertEntry(RTMP_ADAPTER *pAd, UCHAR tr_tb_idx,
			MAC_TABLE_ENTRY *pEntry);
VOID TRTableInsertMcastEntry(RTMP_ADAPTER *pAd, UCHAR tr_tb_idx,
			     struct wifi_dev *wdev);
VOID TRTableEntryDump(RTMP_ADAPTER *pAd, INT tr_idx, const RTMP_STRING *caller,
		      INT line);
...
MAC_TABLE_ENTRY *MacTableLookup2(RTMP_ADAPTER *pAd, UCHAR *pAddr,
				 struct wifi_dev *wdev);
BOOLEAN MacTableDeleteEntry(RTMP_ADAPTER *pAd, USHORT wcid, UCHAR *pAddr);
MAC_TABLE_ENTRY *MacTableInsertEntry(IN RTMP_ADAPTER *pAd, IN UCHAR *pAddr,
				     IN struct wifi_dev *wdev,
				     IN UINT32 ent_type, IN UCHAR OpMode,
				     IN BOOLEAN CleanAll);
...
int read_reg(struct _RTMP_ADAPTER *ad, UINT32 base, UINT16 offset,
	     UINT32 *value);

#ifdef MT_MAC
VOID StatRateToString(RTMP_ADAPTER *pAd, CHAR *Output, UCHAR TxRx,
		      UINT32 RawData);
#endif /* MT_MAC */
...
VOID mac_entry_lookup(RTMP_ADAPTER *pAd, UCHAR *pAddr, struct wifi_dev *wdev,
...
#if defined(INTERNAL_CAPTURE_SUPPORT) || defined(WIFI_SPECTRUM_SUPPORT)
INT32 Set_RBIST_Switch_Mode(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);
INT32 Set_RBIST_Capture_Start(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);
INT32 Get_RBIST_Capture_Status(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);
INT32 Get_RBIST_Raw_Data_Proc(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);
INT32 Get_RBIST_IQ_Data(IN RTMP_ADAPTER *pAd, IN PINT32 pData,
			IN PINT32 pDataLen, IN UINT32 IQ_Type,
			IN UINT32 WF_Num);
INT32 Get_RBIST_IQ_Data_Proc(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);
UINT32 Get_System_CapNode_Info(IN RTMP_ADAPTER *pAd);
UINT32 Get_System_CenFreq_Info(IN RTMP_ADAPTER *pAd, IN UINT32 CapNode);
UINT8 Get_System_Bw_Info(IN RTMP_ADAPTER *pAd, IN UINT32 CapNode);
INT32 Get_System_Wireless_Info(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);
#endif /* defined(INTERNAL_CAPTURE_SUPPORT) || defined(WIFI_SPECTRUM_SUPPORT) */
...
#if defined(MT7615) || defined(MT7622) || defined(P18) || defined(MT7663)
typedef enum _Stat_Action { RESET_COUNTER = 0, SHOW_RX_STATISTIC } Stat_Action;

INT Set_Rx_Vector_Control(RTMP_ADAPTER *pAd, RTMP_STRING *arg,
			  RTMP_IOCTL_INPUT_STRUCT *wrq);
INT Show_Rx_Statistic(RTMP_ADAPTER *pAd, RTMP_STRING *arg,
		      RTMP_IOCTL_INPUT_STRUCT *wrq);
#endif /* defined(MT7615) || defined(MT7622) */
...
INT TxPowerPercentCtrl(PRTMP_ADAPTER pAd, BOOLEAN fgTxPowerPercentEn,
		       UCHAR ucBandIdx);
...
INT TxPowerBfBackoffCtrl(PRTMP_ADAPTER pAd, BOOLEAN fgTxBFBackoffEn,
			 UCHAR ucBandIdx);
INT TxPowerBfBackoffParaCtrl(PRTMP_ADAPTER pAd, UCHAR ChannelBand,
			     UCHAR ControlChannel, UCHAR ucBandIdx);
#else
#endif /* defined(MT7615) || defined(MT7622) */
#endif /* SINGLE_SKU_V2 */
...
INT TxPowerShowInfo(PRTMP_ADAPTER pAd, UCHAR ucTxPowerInfoCatg,
		    UINT8 ucBandIdx);
...
INT BFNDPATxDCtrlCmd(PRTMP_ADAPTER pAd, BOOLEAN fgNDPA_ManualMode,
		     UINT8 ucNDPA_TxMode, UINT8 ucNDPA_Rate, UINT8 ucNDPA_BW,
...
#ifdef TPC_SUPPORT
INT TxPowerTpcFeatureCtrl(PRTMP_ADAPTER pAd, struct wifi_dev *wdev,
			  INT8 TpcPowerValue);
INT TxPowerTpcFeatureForceCtrl(PRTMP_ADAPTER pAd, INT8 TpcPowerValue,
			       UINT8 ucBandIdx, UINT8 CentralChannel);
#endif /* TPC_SUPPORT */
...
#ifdef VENDOR_FEATURE7_SUPPORT
#ifdef DOT11_N_SUPPORT
INT Show_HtBw_Proc(IN PRTMP_ADAPTER pAd, OUT RTMP_STRING *pBuf,
		   IN ULONG BufLen);
INT Show_HtExtcha_Proc(IN PRTMP_ADAPTER pAd, OUT RTMP_STRING *pBuf,
		       IN ULONG BufLen);
#endif
#endif
...
VOID APMlmeDeauthReqAction(IN PRTMP_ADAPTER pAd, IN PMLME_QUEUE_ELEM Elem);
#endif
#ifdef PS_QUEUE_INC_SUPPORT
...
INT Set_ScanResults_Proc(IN PRTMP_ADAPTER pAd, IN RTMP_STRING *arg);

INT Set_ApScan_Proc(IN PRTMP_ADAPTER pAd, IN RTMP_STRING *arg);
...
NDIS_STATUS update_mtb_value(RTMP_ADAPTER *pAd, UCHAR profile_id, UINT_32 extra,
			     RTMP_STRING *value);
...
typedef enum _ENUM_PCIE_ASPM_TYPE_T {
	...
	PCIE_ASPM_TYPE_L1,
	PCIE_ASPM_TYPE_NUM,
} ENUM_PCIE_ASPM_TYPE_T,
	*P_ENUM_PCIE_ASPM_TYPE_T;
#endif /* PCIE_ASPM_DYM_CTRL_SUPPORT */
...
VOID parse_RXV_packet_v1(RTMP_ADAPTER *pAd, UINT32 Type, RX_BLK *RxBlk,
			 UCHAR *Data);
VOID parse_RXV_packet_v2(RTMP_ADAPTER *pAd, UINT32 Type, RX_BLK *RxBlk,
			 UCHAR *Data);
...
BOOLEAN SetCsdDebugSet(RTMP_ADAPTER *pAd, UINT8 band_idx, UINT8 g0_value,
		       UINT8 g1_value, UINT8 g2_value);
INT ShowRxRateHistogram(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
PRxVBQElmt RxVBDequeue(IN PRTMP_ADAPTER pAd);

PRxVBQElmt RxVBQueueSearch(IN PRTMP_ADAPTER pAd, IN UINT8 rxv_sn,
			   IN UINT32 Type);

VOID RecordRxVB(RTMP_ADAPTER *pAd, RX_BLK *RxBlk, UCHAR *Data, UINT32 Type);

VOID RxVB_Report_Action(PRTMP_ADAPTER pAd, UCHAR wcid, PRxVBQElmt prxvbqelmt);

UINT16 Checksum16(UINT8 *pData, int len);
UINT16 UdpChecksum16(UINT8 *pData, int len);
#ifdef CONFIG_STEERING_API_SUPPORT
PBLOCKED_STA_ENTRY BlackList_InsertEntry(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr,
					 IN UINT32 BlockTime);

INT BlackList_DeleteEntry(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr);

PBLOCKED_STA_ENTRY BlackList_StaLookup(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr);

PNA_STA_ENTRY NAStaList_InsertEntry(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr);

INT NAStaList_DeleteEntry(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr);

PNA_STA_ENTRY NaStaList_Lookup(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr);

INT NaStaReportListInsertEntry(IN PRTMP_ADAPTER pAd, IN PCHAR Rssi,
			       IN VOID *Msg, IN ULONG MsgLen, IN UCHAR *Index,
			       IN PUCHAR pAddr, IN struct wifi_dev *wdev);

INT SendBtmReqToAir(IN PRTMP_ADAPTER pAd, IN PUCHAR PeerMACAddr,
		    IN BTM_REQ_FRAME_DATA BtmReqFramedata);

INT Delete_Btm_Action_Frame_Request(IN PRTMP_ADAPTER pAd, IN UCHAR *Addr);

DECLARE_TIMER_FUNCTION(WaitRemoveStaFromBlackList);

#endif
extern INT Set_PartialScan_Proc(IN RTMP_ADAPTER *pAd, IN RTMP_STRING *arg);
